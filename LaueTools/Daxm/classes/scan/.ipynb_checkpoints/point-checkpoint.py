...
class StaticPointScan(object):
    ...
    def init_spec_file(self):
        ...
        if self.filetype == 'spec':
            ...
            self.spec = logfiler.SpecFile(self.spec_file, filetype='hdf5', onlywirescan=True, collectallscans=False,onlymesh=False, verbose=self.verbose)
            if self.verbose:
                print('self.hdf5scanId',self.hdf5scanId)
                print('self.spec_file (hdf5)', self.spec_file)
            # read single key scan (for the moment)
            self.spec_data = logfiler.Scan_hdf5(self.spec, self.hdf5scanId, verbose=self.verbose)
