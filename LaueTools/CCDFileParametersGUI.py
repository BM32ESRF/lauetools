...
class CCDFileParameters(wx.Dialog):
    ...
    def __init__(self, parent, _id, title, CCDlabel):
        ...
        self.paramdetector = ['CCD label', 'header offset', 'dataformat', 'fliprot', 'saturation Value', 'file extension']
        self.units = ['ascii', 'byte', 'python format', 'operator tag', 'integer', 'ascii']
        ...
        self.allCCD_names = GT.put_on_top_list(('MARCCD165','sCMOS', '----------','PRINCETON', 'VHR_Mar13', '----------'),
                                               self.allCCD_names,
                                               forceinsertion=True)

        self.comboCCD = wx.ComboBox(self.panel, -1, self.CCDLabel, (320, posy - 5), size=(180, -1),
                                            choices=self.allCCD_names, style=wx.CB_READONLY)
        ...
        self.comments = wx.TextCtrl(self.panel, style=wx.TE_MULTILINE,
                                    size=(580, 50), pos=(10, 280))
...
#        self.Centre()
    ...
    def readCCDparams(self):
        ...
        (self.parent.framedim, self.parent.pixelsize, self.saturationvalue,
         self.fliprot, self.headeroffset, self.dataformat,
         self.commentstxt, self.file_extension) = CCDparams

        self.value = (self.CCDLabel, self.headeroffset, self.dataformat,
                      self.fliprot, self.saturationvalue, self.file_extension)
    ...
    def DeleteValues(self, event):
        ...
        """
        for kk, param in enumerate(self.paramdetector):
            self.controltext.append(wx.TextCtrl(self.panel, -1, str(None),
                                                (150, self.posyvalues + 30 * kk),
                                                (150, -1)))
