...
class SimulationPlotFrame(wx.Frame):
    ...
    def __init__(self, parent, _id, title,
                ...
                Size=SIZE_PLOTTOOLS, dirname=os.curdir,
                CCDLabel='MARCCD165', **kwds):
        r"""
        ...
        for parentgrainindex in range(nb_ParentGrains):
            ...
            ParentGrainIndex, nb_transforms, transformtype = self.list_grains_transforms[parentgrainindex]
...
#         self.Bind(wx.EVT_BUTTON, self.OnSavePlot, id=5)
        ...
        self.txtctrldistance_angle = wx.StaticText(self.panel, -1,
                                                   '==> %s deg' % '', size=(80, -1))
        self.txtctrldistance_pixel = wx.StaticText(self.panel, -1,
                                                   '==> %s pixel' % '', size=(80, -1))
...
#         self.cidmotion = self.fig.canvas.mpl_connect('motion_notify_event', self.onMotion)
        ...
        self.tooltip = wx.ToolTip(tip='Welcome on LaueTools Laue Pattern simulation frame')
...
#         btnSizer.Add(self.pointButton5, 0, wx.BOTTOM | wx.LEFT)
    ...
    def sliderUpdate(self, event):
        ...
        self.factorsize = 10 ** (int(self.slider.GetValue()) / 50. - 1. + np.log10(self.central_value))
        self._replot()
        # print "factor spot size = %f " % self.factorsize
            ...
            if self.pick_distance_mode:
                ...
                if self.nbclick_dist == 2:
                    ...
                    if self.datatype == 'pixels':
                        ...
                        _dist = np.sqrt((spot1[0] - spot2[0]) ** 2 + \
                                        (spot1[1] - spot2[1]) ** 2)
...
#                         print "Angular distance (q1,q2):  %.3f deg " % _dist
                    ...
                    if self.datatype == '2thetachi':
                        ...
                        self.txtctrldistance_angle.SetLabel('==> %s deg' % \
                                            str(np.round(_dist, decimals=3)))
                    if self.datatype == 'pixels':
                        print("RES =", _dist)
                        ...
                        self.txtctrldistance_pixel.SetLabel('==> %s pixels' % \
                                            str(np.round(_dist, decimals=3)))
...
#                     wx.MessageBox(sentence, 'INFO')
    ...
    def onMotion_ToolTip_fromplotrefine(self, event):
        ...
        xdata_theo, ydata_theo, _annotes_theo = self.Data_X, self.Data_Y, self.Data_Miller
...
#             print 'clickX,clickY in onMotion_ToolTip', clickX, clickY
            ...
            for x, y, a in zip(xdata_theo, ydata_theo, _annotes_theo):
                if (clickX - xtol < x < clickX + xtol) and (clickY - ytol < y < clickY + ytol):
                    annotes_theo.append((GT.cartesiandistance(x, clickX, y, clickY), x, y, a))
...
#                 print "len(self.data[0][k])", len(self.data[0][k])
            ...
            self.central_value = 100. / max(self.Idat) / 5.
    ...
    #             print "with index %d and intensity %.1f" % (annote[0], annote[1])
            ...
            if collisionFound_theo:
                ...
                if _distance < xtol:
                    ...
                    self.statusBar.SetStatusText((sttext + "grainindex: %d spotindex: %d " % \
                    ...
                    tip_theo = "x= %.2f " % x + "y= %2f " % y + "E=%.3f " % E + "[h,k,l]=%s" % HKL
...
#         RotYm40 = np.array([[ca, sa], [-sa, ca]])
    ...
    def _replot(self):
        ...
        if self.showFluoDetectorFrame:
            ...
            pt_zaxis_prime = (pt_lt_prime + pt_rt_prime) / 2.
            ...
            arrows = [patches.YAArrow(self.fig, (pt_lb_prime[0], pt_lb_prime[1]), (center_detframe[0], center_detframe[1]),
                              fc='g', width=0.3, headwidth=.9, alpha=0.5),
               patches.YAArrow(self.fig, (pt_zaxis_prime[0], pt_zaxis_prime[1]), (center_detframe[0], center_detframe[1]),
                               fc='r', width=0.3, headwidth=.9, alpha=0.5)]
...
#             self.axes.add_patch(centrefluoframe)
        ...
        if self.ImageArray is not None:
            ...
            self.myplot = self.axes.imshow(self.ImageArray,
                                           interpolation='nearest')

            if not self.data_dict['logscale']:
                norm = mpl.colors.Normalize(vmin=self.data_dict['vmin'],
                                    vmax=self.data_dict['vmax'])
            else:
                norm = mpl.colors.LogNorm(vmin=self.data_dict['vmin'],
                                    vmax=self.data_dict['vmax'])
        ...
        if self.nbGrains > 1:
            colors = GT.JET(np.arange(self.nbGrains) * 1. / ((self.nbGrains - 1)))
        elif self.nbGrains == 1:
        ...
        for grainindex in range(self.nbGrains):
            ...
            colors[grainindex][3] = 0.  # set alpha to 0  i.e. full transparency
            ...
            self.axes.scatter(np.array(self.Data_X[grainindex]) - self.X_offset,
                        ...
                        s=self.func_size_energy(np.array(self.Data_I[grainindex]), self.factorsize),
        ...
        if self.init_plot:
            if self.datatype == '2thetachi':
                ...
                if self.CCDLabel in ('MARCCD165', 'PRINCETON'):
                    ...
                    wx.MessageBox('The camera with label "%s" is not implemented yet'%self.CCDLabel,'info')

        self.axes.set_title('number of grain(s) : %s' % self.nbGrains)
        self.axes.grid(True)

        if self.experimentaldata:  # plot experimental data
            self.axes.scatter(self.experimentaldata[0] - self.X_offset,
                              self.experimentaldata[1] - self.Y_offset,
                        s=self.func_size_intensity(np.array(self.experimentaldata[2]),
                                              .02,
                                              0.,
                                              lin=1),
                        facecolor='r',
...
#         print "datares", self.GrainName_for_Streaking
        ...
        X = self.GrainName_for_Streaking[4][position_in_list:position_in_list + nb_transforms]
        Y = self.GrainName_for_Streaking[5][position_in_list:position_in_list + nb_transforms]
        nb_grains = len(X)
...
#         pp=posX.T.reshape((3,12,5)).transpose((1,0,2))
        ...
        pixX = posX.T.reshape((nbspots_per_Lauepattern, nb_slipsystems, nb_childgrains)).transpose((1, 0, 2))
        pixY = posY.T.reshape((nbspots_per_Lauepattern, nb_slipsystems, nb_childgrains)).transpose((1, 0, 2))
#         print "pixX", pixX
        
        from matplotlib.collections import LineCollection
        
        if nb_slipsystems > 1:
            colors = GT.JET(np.arange(nb_slipsystems) * 1. / ((nb_slipsystems - 1)))
        ...
        for slipsystem_index in range(nb_slipsystems):
            ...
            for spot_index in range(nbspots_per_Lauepattern):
                xmin, xmax = pixX[slipsystem_index][spot_index][0], pixX[slipsystem_index][spot_index][-1]
                ymin, ymax = pixY[slipsystem_index][spot_index][0], pixY[slipsystem_index][spot_index][-1]
                print("spot_index", spot_index)
                print('[xmin, ymin], [xmax, ymax]', [xmin, ymin], [xmax, ymax])
                
                line_segments = LineCollection(([[xmin, ymin], [xmax, ymax]],), color=colors[slipsystem_index])
...
#                                linestyles='solid')
    ...
    def OnSavePlot(self, evt):
        dlg = wx.FileDialog(self, "Saving in png format. Choose a file",
                            self.dirname, "", "*.*", \
...
#             self.Showindices(evt)
    ...
    def updateStatusBar(self, x, y, annote, grain_index, local_spot_index):
        ...
        self.statusBar.SetStatusText(("grainindex: %d local spotindex: %d " % \
                                      (grain_index, local_spot_index) + 
                                    "x= %.2f " % x + 
                                    "y= %2f " % y + 
                                    "E=%.5f " % E + 
...
#             print 'self.mini', self.mini
    ...
    def drawAnnote(self, axis, x, y, annote):
        ...
        if (x, y) in self.drawnAnnotations:
            ...
            t1 = axis.text(x + coef_text_offset * 1, y + coef_text_offset * 3, "%.3f" % (annote[0]), size=8)
            t2 = axis.text(x + coef_text_offset * 1, y - coef_text_offset * 3, "%d %d %d" % (int(annote[1][0]),
                                                 int(annote[1][1]),
                                                int(annote[1][2])),
                                                        size=8)

            m = axis.scatter([x], [y], s=1, marker='d', c='r',
                             zorder=100, edgecolors='None',
                             alpha=.5)  # matplotlib 0.99.1.1
...
#         print "clickX, clickY", clickX, clickY
    ...
    def GetAngularDistance(self, evt):
        ...
        if self.datatype != '2thetachi':
            ...
            self.statusBar.SetStatusText('ERROR: %s' % sentence,
                                    0)
            return
    ...
    def Showindices(self, event):
        ...
        for x, y, a, ind in self._dataANNOTE:
            if  (clickX - xtol < x < clickX + xtol) and  (clickY - ytol < y < clickY + ytol) :
                annotes.append((GT.cartesiandistance(x, clickX, y, clickY), x, y, a, ind))
...
if __name__ == '__main__':
    ...
    data = [[x_1, x_2], [y_1, y_2], [I_1 * 10000, I_2 * 10000], [miller_1, miller_2],
            nb, plottype, exp_data]
    ...
    class App(wx.App):
        ...
        def OnInit(self):
            ...
            frame = SimulationPlotFrame(None, -1, 'Laue Simulation Frame', data=data, dirname=dirname)
