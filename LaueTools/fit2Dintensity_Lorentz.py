...
def lorentzian(height, center_x, center_y, width_x, width_y):
    ...
    return lambda x, y: height / (1. + 4 * (x - center_x) ** 2 / width_x + 4 * (y - center_y) ** 2 / width_y)
...
def fitlorentzian(data):
    ...
    params = moments(data)
    errorfunction = lambda p: ravel(lorentzian(*p)(*indices(data.shape)) -
                                 data)
...
def twodlorentzian(inpars, circle, rotate, vheight):
    ...
    def rotlorentz(x, y):
        ...
        g = height + amplitude / (1. + 4 * (xp - rcen_x) ** 2 / width_x) / (1 + 4 * (yp - rcen_y) ** 2 / width_y)
        return g
    return rotlorentz
...
if __name__ == "__main__":
    ...
    if 0:
        ...
        rotate = 1
        vheight = 1
        data = twodlorentzian(inpars, circle, rotate, vheight)(Xin, Yin) + 10 * random.random(Xin.shape)
        # -------------------------------------
    ...
    start_j, start_i = argmax(dat) // dat.shape[1], argmax(dat) % dat.shape[1]# from maximum intensity in dat
    ...
    params, cov, infodict, errmsg = lorentzfit(dat, err=None, params=startingparams, autoderiv=1, return_all=1, circle=0, rotate=1, vheight=1)
