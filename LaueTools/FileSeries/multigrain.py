...
def matstarlab_to_deviatoric_strain_crystal(matstarlab,
    ...
    if version == 1 :  # only for initially cubic unit cell
        ...
        epsp[3] = -1000.*(dlat[3] - math.pi / 2.) / 2.
        epsp[4] = -1000.*(dlat[4] - math.pi / 2.) / 2.
        epsp[5] = -1000.*(dlat[5] - math.pi / 2.) / 2.

    elif version == 2 :  # for any symmetry of unit cell

        # reference lattice parameters with angles in degrees
        dlat0_deg = np.array(DictLT.dict_Materials[reference_element_for_lattice_parameters][1],
                                            dtype=float)
...
def glide_systems_to_schmid_tensors(n_ref=array([1., 1., 1.]), b_ref=array([1., -1., 0.]), verbose=0):
    # 29May13
    """
...
def matstarlab_to_matdirONDsample3x3(matstarlab, omega0=40.):  # 29May13
    ...
    try:
        for line in f:
            if (i >= linestartspot[gnum]) & (i < lineendspot[gnum]) :
                # print line
                listspot.append(line.rstrip('\n').replace('[', '').replace(']', '').split())

            if (i >= linemat[gnum]) & (i < linemat[gnum] + 3) :
                # print line
                listmat.append(line.rstrip('\n').replace('[', '').replace(']', '').split())
...
def read_xmas_txt_file_from_seq_file(filexmas,
    ...
    try:
        for line in f:
            if i == 0 : npts = line.rstrip("  \n")
            if i == 1 : nameline1 = line.rstrip("\n")
            i = i + 1
            if i > 2 : break
    finally:
        f.close()
...
def hkl_to_xystereo(hkl0, polar_axis=[0., 0., 1.], down_axis=[1., 0., 0.], return_more=None) :
    ...
    if return_more == None :  return(qsxy)
...
if 0 :  # ## test 1  #29May13
    ...
    mat3x3 = array([[1., 0.01, 0.02], [0., 1.05, 0.03], [0., 0., 1.03]])
    print("mat3x3")
    print(mat3x3)
    ...
    epsp1, dlatrdeg = matstarlab_to_deviatoric_strain_crystal(matstarlab,
                                                version=1,
                                                reference_element_for_lattice_parameters="Ge")
    print("version 1 :")
    print("deviatoric strain aa bb cc -dalf bc, -dbet ac, -dgam ab (1e-4 units)")
    print((epsp1 * 10.).round(decimals=2))
    ...
    epsp1, dlatrdeg = matstarlab_to_deviatoric_strain_crystal(matstarlabLT,
                                                version=2,
                                                reference_element_for_lattice_parameters="Ge")
    ...
    epsp1, dlatrdeg = matstarlab_to_deviatoric_strain_crystal(matstarsample,
                                                version=2,
                                                reference_element_for_lattice_parameters="Ge")
    ...
    print("version 2 :")
    epsp_sample_v1 = matstarlab_to_deviatoric_strain_sample(matstarlab,
                                                            omega0=40.0,
                                                            version=2,
                                                            reference_element_for_lattice_parameters="Ge")
    print("deviatoric strain xx yy zz -dalf yz, -dbet xz, -dgam xy (1e-4 units)")
    print((epsp_sample_v1 * 10.).round(decimals=2))
...
def test_index_refine(filedat, paramdetector_top, use_weights=False, proposed_matrix=None, \
              check_grain_presence=None, paramtofit="strain", elem_label="UO2", grainnum=1,
                      remove_sat=0, elim_worst_pixdev=1, maxpixdev=1.0,
                      spot_index_central=[0, 1, 2, 3, 4, 5], nbmax_probed=20, energy_max=22,
                      rough_tolangle=0.5 , fine_tolangle=0.2, Nb_criterium=15,
                      NBRP=1, mark_bad_spots=0, boolctrl='1' * 8,
                      pixelsize=165. / 2048,
                      dim=(2048, 2048),
                        LUT=None):
...
# #    dim1 = (2048,2048)
    ...
    data_twotheta, data_chi = F2TC.calc_uflab(data_pixX, data_pixY, calib, returnAngles=1, pixelsize=pixelsize, signgam=SIGN_OF_GAMMA)
...
# #        data_y1 = data_y1[istart:]
        ...
        for spotnum in spot_index_central :
            ...
            RES_onespot = INDEX.getOrientMatrix_from_onespot(spotnum, rough_tolangle,
                                              start_tab_distance, data[0], data[1],
                                              nLUT, Bmatrix,
                                              LUT=LUT,
...
#        orientmatrix = verybestmat
    ...
    # print spotnum_i_pixdev_2
        ...
        res = LAA.GetCalibParameter(emax_simul, veryclose_angletol, elem_label, \
                    ...
                    pixelsize=pixelsize, dim=dim1, signgam=SIGN_OF_GAMMA, boolctrl=boolctrl, \
...
# #        print "spotnum_hkl_2 \n", spotnum_hkl_2
    ...
    for i in range(nfit_2):
        ...
        # print "npeak = ", npeak
        hkl = spotnum_hkl[i, 1:]
        print(npeak, data_pixX[npeak], data_pixY[npeak], np.array(hkl, dtype=int), data_I[npeak], data_chi[npeak], data_theta[npeak])
    ...
    epsp1, dlatsrdeg1 = matstarlab_to_deviatoric_strain_crystal(GT.mat3x3_to_matline(matLTmin),
                                            version=2,
    ...
    print(latticeparam)
    dlatu = np.array([latticeparam, latticeparam, latticeparam, pi1 / 2.0, pi1 / 2.0, pi1 / 2.0])
    ...
    if paramtofit == "strain" :
        filefit = save_fit_results(filecor, data_fit_sorted , matLTmin, deviatoricstrain, filesuffix, \
                                   pixdev_2, grainnum, paramdetector_top, pixelsize, dim)
    elif paramtofit == "calib" :
        filefit = save_fit_results(filecor, data_fit_sorted , matLTmin, deviatoricstrain, filesuffix, \
                                   pixdev_2, grainnum, newparam, pixelsize, dim)
...
def save_fit_results(filename, data_fit , matmin, deviatoricstrain, filesuffix, pixdev, grainnum, calib, pixelsize=165. / 2048., dim=(2048.0, 2048.0)):
    ...
    header += '# Number of indexed spots : ' + str(shape(data_fit)[0]) + ' pixdev :' + str(round(pixdev, 4)) + '\n'
...
def convert_xmas_str_to_LT_fit(filestr, elem_label="W", stiffness_c_tensor=None, schmid_tensors=None):
    ...
    for i in range(len(npeaks)):
        ...
        if npeaks[i] > 0 :
            ...
            if stiffness_c_tensor != None :
                ...
                print("Von Mises equivalent Stress (100 MPa units)", round(von_mises, 3))
            ...
            filefit = save_fit_results(filecor, data_fit_sorted, matLTmin, deviatoricstrain, filesuffix, dev_str[i, 2], i + 1, calib)
...
def readlt_fit_mg(fitfilename, verbose=1, readmore=False, fileextensionmarker='.fit'):  # 29May13
    """
...
#            data_fit[:, 2:5] = np.dot(transfmat, hkl.transpose()).transpose()
        ...
        # xx yy zz yz xz xy
        # voigt notation
        strain6[k, :] = np.array([strain[0, 0], strain[1, 1], strain[2, 2],
...
#             print "iline =%d line" % iline, line
            ...
            elif line.startswith(('# Mean Pixel Deviation', '# Mean Deviation(pixel):')):
...
#                 print "got calibration parameters:", calibparam
            ...
            if calibfound & (iline == linecalib):
                calibJSM[grain_index, :5] = np.array(line.rstrip('\n').replace('[', '').replace(']', '').split(',')[:5], dtype=float)
...
#            data_fit[:, 2:5] = np.dot(transfmat, hkl.transpose()).transpose()
    ...
    if readmore == False :
        return(gnumlist, npeaks, indstart, matstarlab, data_fit_all,
               calibJSM[:, :5], pixdev)
    elif readmore == True :
        return(gnumlist, npeaks, indstart, matstarlab, data_fit_all,
               calibJSM[:, :5], pixdev, strain6, euler)
...
def compare_multigrain_fit(filefitmg1, filefitmg2, mat_tol=1.0e-3, dxytol=0.05, compare_calib=0, elem_label="W"):
...
if 0 :  # # test 4
    mat = array([[ 0.17393303, 0.53947968, 0.82594746], [-0.78117538, 0.58454443, -0.21760663], \
...
if 0 :  # ## test 6b
    ...
    compare_multigrain_fit(filefitGe1, filefitGe2, mat_tol=5.0e-3, dxytol=0.01, compare_calib=1)
    jkldqs
...
if 0 :  # ## test 7b
    ...
    filefitGe2 = filepath + "Ge_v1b_from_xmas_1.fit"
    compare_multigrain_fit(filefitGe1, filefitGe2, mat_tol=5.0e-3, dxytol=1.0, compare_calib=1)
    jklqsdqs
...
if 0 :  # ## test 8b
    ...
    compare_multigrain_fit(filefitW1, filefitW2, mat_tol=5.0e-3, dxytol=0.01, compare_calib=0)
    jdslqqds
# #
if 0 :  # ## test 9
    filestr = filepath + "W_v3c.str"
...
if 0 :  # ## test 9b
    ...
    filefitW2 = filepath + "W_v3c_from_xmas_mg.fit"
    compare_multigrain_fit(filefitW1, filefitW2, mat_tol=5.0e-3, dxytol=0.1, compare_calib=0)
    jdslqqds
...
if 0 :  # ## test 11
    ...
    filefitW1_new = filepath + "Wmap_WB_14sep_d0_500MPa_0045_LT_0f_from_dat_UWN_mg_calibXMAS.fit"
    os.rename(filefitW1, filefitW1_new)
    jklsdqjsqdl

if 0 :  # ## test 11b
    filefitW1 = filepath + "Wmap_WB_14sep_d0_500MPa_0045_LT_0f_from_dat_UWN_mg_calibXMAS.fit"
    filefitW2 = filepath + "W_v3c_from_xmas_mg.fit"
    compare_multigrain_fit(filefitW1, filefitW2, mat_tol=5.0e-3, dxytol=1.0, compare_calib=0)
    jklfdsjkl
...
if 0 :  # ## test 12
    ...
    filefitW1 = filepath + "Wmap_WB_14sep_d0_500MPa_0045_LT_0_from_dat_UWN_mg.fit"
    filefitW1_new = filepath + "Wmap_WB_14sep_d0_500MPa_0045_LT_0f_from_dat_UWN_mg_calibLT.fit"
    os.rename(filefitW1, filefitW1_new)
    jklsdqjsqdl

if 0 :  # ## test 12b
    filefitW1 = filepath + "Wmap_WB_14sep_d0_500MPa_0045_LT_0f_from_dat_UWN_mg_calibLT.fit"
    filefitW2 = filepath + "W_v3c_from_xmas_mg.fit"
    compare_multigrain_fit(filefitW1, filefitW2, mat_tol=5.0e-3, dxytol=1.0, compare_calib=1)
    jklfdsjkl
...
def serial_peak_search(filepathim, fileprefix, indimg, filesuffix, filepathout) :
    ...
    for i in indimg :
        print("i = ", i)
        # filename = filelist[i]
        filename = os.path.join(filepathim, fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name) + filesuffix)
        print("image in :")
        print(filename)
        print("saving peak list in :")
        fileprefix1 = os.path.join(filepathout, fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name))
        filedat = fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name) + ".dat"
        import module_graphique as modgraph
        ...
        if not PAR.overwrite_peak_search :
            while (filedat in os.listdir(filepathout)):
                ...
                fileprefix2 = fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name) + "_new_" + str(j)
        ...
        Isorted, fitpeak, localpeak, to_reject3 = rmccd.PeakSearch(filename,
        ...
        if shape(Isorted)[0] > 0 :
            ...
            IOLT.writefile_Peaklist(fileprefix1,
                                    Isorted,
                                    overwrite=1,
...
def index_refine_multigrain_one_image(filedat1,
    ...
    matstarlab, data_fit, calib, pixdev = F2TC.readlt_fit(filefitcalib, readmore=True, verbose=1)
...
# uses a number of invisible parameters set in param.py
def serial_indexerefine_multigrain(filepathdat,
                                   fileprefix,
                                   indimg,
                                   filesuffix,
                                   filefitcalib,
    ...
    for i in indimg :
        print("i = ", i)
        # filename = filelist[i]
        filedat1 = os.path.join(filepathdat, fileprefix + \
                    rmccd.stringint(i, PAR.number_of_digits_in_image_name) + \
                    filesuffix)
        print("image in :")
        print(filedat1)
        # print "saving fit in :"
        filefit = fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name) + PAR.add_str_index_refine + ".fit"
        import module_graphique as modgraph
       ...
       # print os.listdir(filepathout)
        ...
        if not PAR.overwrite_index_refine :
            while (filefit in os.listdir(filepathout)):
                print("warning : change name to avoid overwrite")
                filefit = fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name) + PAR.add_str_index_refine + "_new_" + str(j) + ".fit"
        ...
        filefitmg, ngrains_found[k], npeaks[k, :], pixdev[k, :] = \
...
def serial_index_refine_multigrain_v2(filepathdat,
                                   fileprefix,
                                   indimg,
                                   filesuffix,
                                   filefitcalib,
    ...
    if filefitref != None :
        matstarlab, data_fit, calib1, pixdev = F2TC.readlt_fit(filefitref, readmore=True)
        matstarlabOND = matstarlab_to_matstarlabOND(matstarlab)
        matstarlab1 = matstarlabOND * 1.0
...
#                print res1
        if res1 != 0 :
                gnumlist, npeaks1, indstart, matstarlab_all, data_fit1, calib1, pixdev1, strain6, euler = res1
    ...
    for i in indimg :
        print("i = ", i)
        # filename = filelist[i]
        filedat1 = filepathdat + fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name) + filesuffix
        print("image in :")
        print(filedat1)
        # print "saving fit in :"
        filefit = fileprefix + rmccd.stringint(i, PAR.number_of_digits_in_image_name) + PAR.add_str_index_refine + ".fit"
        ...
        res1 = \
        index_refine_multigrain_one_image_with_twins(filedat1,
                                          calib,
                                          ngrains=PAR.ngrains_index_refine,
                                          proposed_matrix=proposed_matrix)

        print("k = ", k)
        if PAR.ngrains_index_refine > 1 : filefitmg, ngrains_found[k], npeaks[k, :], pixdev[k, :] = res1
        else :  filefitmg, ngrains_found[k], npeaks[k], pixdev[k] = res1
...
if 0 :  # ## test 14 : serial index refine multigrain
    ...
    ngrains_found, npeaks, pixdev = serial_indexerefine_multigrain(filepathdat, fileprefix, indimg, filesuffix, filefitcalib, filepathout)

    jkldasdsa

# uses a number of invisible parameters set in param.py
def index_refine_calib_one_image(filedat1, filedet=None, filefitcalib=None, \
                                 boolctrl='1' * 8, fixedcalib=None,
                                 elim_worst_pixdev=1,
                                 maxpixdev=0.7,
                                 elem_label="Ge",
                                 CCD_label="MARCCD165"):
    ...
    filefit, filecor, npeaks_LT, pixdev_LT, matLTmin, calibLT = \
            res1 = test_index_refine(filedat1,
                                 paramdetector_top,
                                 use_weights=False,
                                 proposed_matrix=matLT3x3,
...
# uses a number of invisible parameters set in param.py
def serial_index_refine_calib(filepathdat, fileprefix, indimg, filesuffix, \
                              filefitcalib, filepathout, boolctrl='1' * 8,
                              fixedcalib=None, fixed_filefitcalib="yes",
                              number_of_digits_in_image_name=4,
                              add_str_index_refine="_t_UWN"):  # string to add : UWN = "use weights no")
    nimg = len(indimg)
    ...
    for i in indimg :
        print("i = ", i)
        # filename = filelist[i]
        filedat1 = filepathdat + fileprefix + \
        ...
        if not PAR.overwrite_index_refine :
            while (filefit in os.listdir(filepathout)):
                ...
                filefit = fileprefix + \
                                rmccd.stringint(i, number_of_digits_in_image_name) + \
                                        add_str_index_refine + "_new_" + str(j) + ".fit"
                print(filepathout + filefit)
                j = j + 1
...
if 0 :  # # test 15
    ...
    ngrains_found, npeaks, pixdev = serial_indexerefine_multigrain(filepathdat, fileprefix, indimg, filesuffix, filefitcalib, filepathout, filefitref=None)
...
if 0 :  # test filter_peaks
    filepath = "D:\\Documents and Settings\\or208865\\Bureau\\mono_inverse\\Nov12\\Gedia1\\"
...
def get_xyzech(filepathim, fileprefix, indimg, filesuffix, filepathout):
    ...
    for k in indimg:
            ...
            print(fileprefix)
            print(filesuffix)
            fileim = os.path.join(filepathim, fileprefix + rmccd.stringint(k, PAR.number_of_digits_in_image_name) + filesuffix)
            print(fileim)
...
# #            data_list[kk,5] = float(toto1)
    ...
    try:
        ...
        outfilename = os.path.join(modgraph.outfilenamexyz, fileprefix + str(modgraph.indimg[0]) + '_to_' + str(modgraph.indimg[-1]) + '.dat')
        print(outfilename)
...
def build_xy_list_by_hand(fileprefix,
                          nx, ny,
                          xfast, yfast,
                          xstep, ystep,
    ...
    outputfilename = os.path.join(dirname,
                fileprefix + '%s_to_%s.dat' % (str(startindex), str(lastindex)))
...
#         res1 = readlt_fit_mg(filefitmg, verbose=1, readmore=True)
        ...
        res1 = readfitfile_multigrains(filefitmg, verbose=1,
                                    readmore=True,
...
#         print "res1", res1
        ...
        if res1 != 0:
            (gnumlist, npeaks, indstart,
             matstarlab, data_fit, calib,
             pixdev, strain6, euler) = res1
            ...
            # print shape(dxylist)
            res = column_stack((imnumlist, gnumlist, npeaks,
                                 pixdev, intensity, dxylist,
                                 matstarlab, strain6, euler))
    ...
    print(shape(allres))
    print(folderoutput)
    print(fileprefix + '%s%s_to_%s.dat' % (outputprefix,
                                            str(startindex),
                                            str(finalindex)))
    ...
    try:
        ...
        fullpath_summary_filename = os.path.join(folderoutput,
                    fileprefix + '%s%s_to_%s.dat' % (outputprefix,
                                                    str(startindex),
                                                    str(finalindex)))
...
def twomat_to_rotation_Emeric(matstarlab1, matstarlab2, omega0=40.):
...
def add_columns_to_summary_file_new(filesum,
    ...
    if include_strain :
        ...
        list_col_names_strain = ["strain6_crystal", "strain6_sample",
                      "stress6_crystal", "stress6_sample", "res_shear_stress", "max_rss", "von_mises" ]
    ...
    if include_misorientation:
        ...
        if filefitref_for_orientation == None :
            ...
            if filter_mean_matrix_by_pixdev_and_npeaks :
                ...
                indfilt = where((pixdev_list < maxpixdev_for_mean_matrix) & (npeaks_list > minnpeaks_for_mean_matrix))
                ...
                indfilt = where((intensity_list > minintensity_for_mean_matrix) & (npeaks_list > 0.0))
                matstarlabref = (mat_list[indfilt[0]]).mean(axis=0)
                print("number of points used to calculate matmean", len(indfilt[0]))
...
#        matmean3x3 = GT.matline_to_mat3x3(matmean)
    ...
    for i in range(numig):
        ...
        if npeaks_list[i] > 0.0:
            ...
            calc_cosines_first_stereo_triangle(matstarlab, xsample_sample_coord)
            rgb_xlab[i, :] = rgb_x[i, :] * 1.
            # print "y"
            matstarlabnew, transfmat, rgb_y[i, :] = \
            calc_cosines_first_stereo_triangle(matstarlab, ysample_sample_coord)
            ...
            calc_cosines_first_stereo_triangle(matstarlab, zsample_sample_coord)
...
#                vec_crystal, vec_lab, misorientation_angle[i] = twomat_to_rotation(matmean3x3,mat2, verbose = 0)
                ...
                (vecRodrigues_sample,
                 misorientation_angle[i]) = twomat_to_rotation_Emeric(matstarlabref,
                                                                      matstarlab,
                                                                      omega0=omega_sample_frame)
...
#                if k == 5 : return()
    ...
    data_list = column_stack((data_1[:numig, :], \
                              rgb_x, rgb_y, rgb_z, rgb_xlab, rgb_ylab, rgb_zlab))

    if include_strain :
        data_list = column_stack((data_list, epsp_crystal, epsp_sample, \
                    sigma_crystal, sigma_sample, tau1, maxrss, von_mises))
...
def add_columns_to_summary_file(filesum, elem_label="Ge", filestf=None, verbose=0):  # 29May13
    ...
    list_col_names = ["strain6_sample", "rgb_x_sample", "rgb_z_sample", "stress6_crystal",
                      "stress6_sample", "res_shear_stress", "max_rss", "von_mises" ]
...
max_rss 61, von_mises 62 \n"
    ...
    axis_pole_sample_z = array([0., 0., 1.])
    axis_pole_sample_x = array([1., 0., 0.])
    ...
    for i in range(numig):
        ...
        if data_1[i, 2] > 0.0:
            ...
            # print "z"
            matstarlabnew, transfmat, rgb_z[i, :] = \
            calc_cosines_first_stereo_triangle(matstarlab, axis_pole_sample_z)
            # print "x"
            matstarlabnew, transfmat, rgb_x[i, :] = \
            calc_cosines_first_stereo_triangle(matstarlab, axis_pole_sample_x)

            epsp_sample[i, :], epsp_crystal = matstarlab_to_deviatoric_strain_sample(matstarlab,
                                                    omega0=40.0,
                                                    version=2,
                                                    returnmore=True,
                                                    reference_element_for_lattice_parameters=elem_label)
    ...
    data_list = column_stack((data_1[:numig, :], epsp_sample, rgb_x, rgb_z,
                              sigma_crystal, sigma_sample, tau1, maxrss, von_mises))
...
def plot_orientation_triangle_color_code():
    ...
    for i in range(numrand1 + 1):
        for j in range(numrand1 + 1):
            col1 = zeros(3, float)
            uq = (1. - range001[i]) * uqref_cr[:, 0] + range001[i] * (angrange[j] * uqref_cr[:, 1] + (1. - angrange[j]) * uqref_cr[:, 2])
            uq = uq / norme(uq)
...
#        list_column_names =  ['img', 'gnumloc', 'npeaks', 'pixdev']
        ...
        if filter_on_pixdev_and_npeaks :
            ...
            ind1 = where((gnumlist == grain_index) & (pixdevlist < maxpixdev_forfilter) & (npeakslist > minnpeaks_forfilter))
        else :
        ...
        if maptype == "euler3" :
        ...
        elif maptype == "rgb_x_sample": rgbxz = data_list2[:, indcolplot]
            ...
            if maptype in ['max_rss', 'von_mises']:
                for j in range(ncolplot) : plotdat[:, :, 3 * j] = 1.0
            else :
                for j in range(ncolplot) : plotdat[:, :, 3 * j:3 * (j + 1)] = 0.5
        ...
        for i in range(numig2) :
            ...
            elif maptype == "rgb_x_sample":  plotdat[iref, jref, :] = rgbxz[i, :] * 1.0

            elif maptype == "fit":
                plotdat[iref, jref, 0:3] = ((npeakslist2[i] - npeaksmin_forplot) / (npeaksmax_forplot - npeaksmin_forplot)).clip(min=0.0, max=1.0)
                plotdat[iref, jref, 3:6] = ((pixdevmax_forplot - pixdevlist2[i]) / (pixdevmax_forplot - pixdevmin_forplot)).clip(min=0.0, max=1.0)

            elif maptype in ['max_rss', 'von_mises']:
                plotdat[iref, jref, 0:3] = (strain62[i, j] - stramin[j]) / (stramax[j] - stramin[j]).clip(min=0.0, max=1.0)
            else:
                for j in range(ncolplot):
                    toto = (strain62[i, j] - stramin[j]) / (stramax[j] - stramin[j]).clip(min=0.0, max=1.0)
...
# #                    plotdat[iref, jref, :] = array([0., 0., 1.])
        ...
        for j in range(nb_plots):
            ...
            plo = ImshowFrameNew(None, -1, strname, plotdat[:, :, 3 * j:3 * (j + 1)],
...
DEFAULT_PLOTMAPS_PARAMETERS_DICT = {
            'Map Summary File':None,
             ...
             'probed_grainindex':0,
             'filter_on_pixdev_and_npeaks':1,
             'maxpixdev_forfilter':20.,  # only for filter_on_pixdev_and_npeaks : 1
             'minnpeaks_forfilter':1.,  # only for filter_on_pixdev_and_npeaks : 1
             ...
             'pixdevmax_forplot':10.,  # only for maptype : "fit"
...
def plot_map_new2(dict_params, maptype, grain_index, App_parent=None):  # JSM May 2017
        ...
        list_column_names = ['img', 'probed_grainindex', 'npeaks', 'pixdev', 'intensity',
                                        'dxymicrons_0', 'dxymicrons_1',
             'matstarlab_0', 'matstarlab_1', 'matstarlab_2', 'matstarlab_3', 'matstarlab_4', # 7:16
             'matstarlab_5', 'matstarlab_6', 'matstarlab_7', 'matstarlab_8',
             'strain6_crystal_0', 'strain6_crystal_1', 'strain6_crystal_2', 'strain6_crystal_3', # 16:22
             'strain6_crystal_4', 'strain6_crystal_5',
             'euler3_0', 'euler3_1', 'euler3_2', # 22:25
             'strain6_sample_0', 'strain6_sample_1', 'strain6_sample_2', 'strain6_sample_3',
             'strain6_sample_4','strain6_sample_5',
             'rgb_x_sample_0', 'rgb_x_sample_1', 'rgb_x_sample_2', #25:31
             'rgb_z_sample_0', 'rgb_z_sample_1', 'rgb_z_sample_2',
             'stress6_crystal_0', 'stress6_crystal_1', 'stress6_crystal_2', # 31:37
             'stress6_crystal_3', 'stress6_crystal_4','stress6_crystal_5',
             'stress6_sample_0', 'stress6_sample_1', 'stress6_sample_2', # 37:43
             'stress6_sample_3', 'stress6_sample_4', 'stress6_sample_5',
             'res_shear_stress_0', 'res_shear_stress_1', 'res_shear_stress_2', 'res_shear_stress_3', # 43:15
             'res_shear_stress_4', 'res_shear_stress_5', 'res_shear_stress_6', 'res_shear_stress_7',
             'res_shear_stress_8', 'res_shear_stress_9', 'res_shear_stress_10', 'res_shear_stress_11',
             'max_rss', 'von_mises', # 58 and # 59
             'misorientation_angle', 'dalf']
        ...
        dict_nplot = {
            "euler3" : [3, 3, 1, 1, 1, 0, ["rgb_euler", ]],
            "rgb_x_sample" : [9, 9, 3, 1, 3, 0, ["x_sample", "y_sample", "z_sample","x_sample", "y_sample", "z_sample","x_sample", "y_sample", "z_sample"]],
            "orientation" : [9, 9, 3, 1, 3, 0, ["x_sample", "y_sample", "z_sample","x_sample", "y_sample", "z_sample","x_sample", "y_sample", "z_sample"]],
...
#         print "z_values.shape",z_values.shape
        ...
        for index_component in range(nbdatacolumns):
            ...
            if maptype == 'orientation':
                ...
                print("z_values.shape",z_values.shape)
                plot_maptype_list[index_component]=str(list_vecs[index_component/3])+plot_maptype_list[index_component]
                ...
                zvalues = zvalues_Ncomponents[:,index_component*3:(index_component+1)*3]
...
def plot_map_new(dict_params, App_parent=None):  # 29May13
        ...
        list_column_names = ['img', 'probed_grainindex', 'npeaks', 'pixdev', 'intensity',
                                        'dxymicrons_0', 'dxymicrons_1',
             'matstarlab_0', 'matstarlab_1', 'matstarlab_2', 'matstarlab_3', 'matstarlab_4', 'matstarlab_5',
             'matstarlab_6', 'matstarlab_7', 'matstarlab_8',
             'strain6_crystal_0', 'strain6_crystal_1', 'strain6_crystal_2', 'strain6_crystal_3', 'strain6_crystal_4',
             'strain6_crystal_5',
             'euler3_0', 'euler3_1', 'euler3_2',
             'strain6_sample_0', 'strain6_sample_1', 'strain6_sample_2', 'strain6_sample_3', 'strain6_sample_4',
             'strain6_sample_5',
             'rgb_x_sample_0', 'rgb_x_sample_1', 'rgb_x_sample_2',
             'rgb_z_sample_0', 'rgb_z_sample_1', 'rgb_z_sample_2',
             'stress6_crystal_0', 'stress6_crystal_1', 'stress6_crystal_2', 'stress6_crystal_3', 'stress6_crystal_4',
             'stress6_crystal_5',
             'stress6_sample_0', 'stress6_sample_1', 'stress6_sample_2', 'stress6_sample_3', 'stress6_sample_4',
             'stress6_sample_5',
             'res_shear_stress_0', 'res_shear_stress_1', 'res_shear_stress_2', 'res_shear_stress_3', 'res_shear_stress_4', 'res_shear_stress_5', 'res_shear_stress_6', 'res_shear_stress_7', 'res_shear_stress_8', 'res_shear_stress_9', 'res_shear_stress_10', 'res_shear_stress_11',
        ...
        if d['col_for_simple_map'] == None:
            ...
            if 'misorientation_angle' in listname:
                ...
                if filter_mean_strain_on_misorientation :
                    ...
                    print("filtering out img with large misorientation > ", d['max_misorientation'])
...
#            if maptype in ['max_rss','von_mises','misorientation_angle', 'dalf', "intensity"]:
        ...
        if abs(d['map_rotation']) > 0.1:
            ...
            filexyz_new, xylim_new = rotate_map(filexyz,
                                                d['map_rotation'],
                                                xylim=d['xylim'])
        ...
        if d['filter_on_pixdev_and_npeaks']:
            ...
            indf = where((gnumlist == d['probed_grainindex']) & \
...
#            if maptype in ['max_rss','von_mises','misorientation_angle', "intensity"]:
            if nb_values == 1:
                ...
                for j in range(nb_values):
                    plotdat[:, :, 3 * j:3 * (j + 1)] = 0.  # black = color for missing data
                if maptype != "dalf":
                    if maptype != "w_mrad":
                        print("xx xy xz yy yz zz")
                    else:
...
#                    if maptype in ['max_rss','von_mises','misorientation_angle', "intensity"]:
            ...
            if (maptype == 'misorientation_angle') & (d['use_mrad_for_misorientation'] == "yes"):
                print("converting misorientation angle into mrad")
                list_plot = list_plot * np.pi / 180. * 1000.
...
#                 print "npeaksmin_forplot", npeaksmin_forplot
                ...
                if d['low_npeaks_as_red_in_npeaks_map'] != None:
                    if (npeakslist2[i] < d['low_npeaks_as_red_in_npeaks_map']):
                        plotdat[iref, jref, 0:3] = array([1., 0., 0.])
                else:
...
#                elif maptype in ['max_rss','von_mises', "misorientation_angle", "intensity"]:
                ...
                datarray_info[iref, jref, :] = [val_singlevalue, val_singlevalue, val_singlevalue]
                ...
                for j in range(nb_values):
                    if list_plot[i, j] > list_plot_max[j] :
                        plotdat[iref, jref, 3 * j:3 * j + 3] = d['color_for_max_strain_positive']
                    elif list_plot[i, j] < list_plot_min[j] :
                        plotdat[iref, jref, 3 * j:3 * j + 3] = d['color_for_max_strain_negative']
                    else :
...
#    klmdqs
    ...
    ig_list = column_stack((np.array(sorted_list[:, :3], dtype=int), \
                         is_ref, has_grain_num, global_gnum, grain_size, list(range(nmax))))
    ...
    try:
        for line in f:
            for j in range(ndict):
                if line.rstrip('\n') == dict_values_names[j] : linepos_list[j] = i
            i = i + 1
    finally:
        f.close()
...
def fill_dict_grains(filesum, filegrains):  # 29May13
    ...
    list_column_names_to_add = ['npeaks', 'pixdev', 'intensity',
 'strain6_crystal_0', 'strain6_crystal_1', 'strain6_crystal_2', 'strain6_crystal_3', 'strain6_crystal_4', 'strain6_crystal_5',
 'strain6_sample_0', 'strain6_sample_1', 'strain6_sample_2', 'strain6_sample_3', 'strain6_sample_4', 'strain6_sample_5',
 'rgb_x_sample_0', 'rgb_x_sample_1', 'rgb_x_sample_2',
 'rgb_z_sample_0', 'rgb_z_sample_1', 'rgb_z_sample_2',
...
def add_intragrain_rotations_to_dict_grains(filesum, filegrains):  # 29May13
    ...
    dict_grains, dict_values_names = read_dict_grains(filegrains, dict_with_edges="yes", dict_with_all_cols="yes")
...
#    print len(ind_gnum)
    ...
    if mapcolor == "gnumloc_in_grain_list" :
        if gnumloc_min == 0 :  # XMAS
            ...
            dict_rgb_gnumloc = {1:[1., 0., 0.], 2: [0., 1., 0.], 3:[0.5, 0.5, 1.], 4:[0.5, 0.5, 0.5] }
    ...
    'stress6_crystal_3', 'stress6_crystal_4', 'stress6_crystal_5',
    'stress6_sample_0', 'stress6_sample_1', 'stress6_sample_2',
    'stress6_sample_3', 'stress6_sample_4', 'stress6_sample_5'] : dict_dec[key] = [3, 2]

    for key in ["rgb_x", "rgb_z"] : dict_dec[key] = [3, 3]
    for key in ['max_rss', 'von_mises', 'misorientation_angle'] : dict_dec[key] = [3, 0]


    if filter_on_pixdev_and_npeaks == "yes" : filter_str = "_filter"
    else : filter_str = ''
...
#    dict_grains, dict_values_names = read_dict_grains(filegrains, dict_with_edges = "yes")
    ...
    if mapcolor in ["rgb_x", "rgb_z"]:
        ...
        if minvalue_for_plot == None : minvalue_for_plot = minvalue
        if maxvalue_for_plot == None : maxvalue_for_plot = maxvalue
...
#        list_col = zeros(nimg, dtype=int)
        ...
        if number_of_graphs == None : rgb1 = color_for_missing_data * ones(shape_rgb, dtype=float)
        ...
        for i in range(nimg):
            ...
            if maptype == 0 :
                rgb1[list_line[i], list_col[i], :] = ((list_plot[i] - minvalue_for_plot) / (maxvalue_for_plot - minvalue_for_plot)).clip(min=0.0, max=1.0)
            elif maptype == 1 :
                rgb1[list_line[i], list_col[i], :] = ((maxvalue_for_plot - list_plot[i]) / (maxvalue_for_plot - minvalue_for_plot)).clip(min=0.0, max=1.0)
            elif maptype == 2 :
                toto = ((list_plot[i] - minvalue_for_plot) / (maxvalue_for_plot - minvalue_for_plot)).clip(min=0.0, max=1.0)
                rgb1[list_line[i], list_col[i], :] = np.array(cmap(toto))[:3]
                if list_plot[i] > maxvalue_for_plot : rgb1[list_line[i], list_col[i], :] = [1.0, 1.0, 0.0]  # yellow
                elif list_plot[i] < minvalue_for_plot : rgb1[list_line[i], list_col[i], :] = [1.0, 0.0, 0.0]  # red
            elif maptype == 3 :
            ...
            if number_of_graphs_per_figure > 1 :
                if not(kk % number_of_graphs_per_figure) :
                    if kk > 0 :
                        ...
                        kkend = kk - 1
                        figfilename = os.path.join(filepathout, str(map_prefix) + str(k) + "_g_" + str(kkstart) + "_to_" + str(kkend) + ".png")
                        try:
                            import module_graphique as modgraph
                ...
                ax = p.subplot(dict_graph[number_of_graphs_per_figure][0], dict_graph[number_of_graphs_per_figure][1], gnum1to9)
...
#                        else :
            ...
            if number_of_graphs_per_figure > 1:
                if zoom == "yes":
                    ...
                    p.text(xmin1 + dxystep_abs[0] / 2.0, ymin1 + dxystep_abs[1] / 2.0, str(gnum), ha="center", va="center")
                    p.xlim(xmin1, xmax1)
                    p.ylim(ymin1, ymax1)
                if zoom == "no":
...
if 0 :  # creation du fichier resume des positions des 4 pics pour toutes les images
    ...
    hkl4C = array([[ 4. , 0., -2.], [ 2. , 0. , 0.], [ 5., -1., -1.], [ 4., -2., -4.]])
    xy4C = array([[  878.241 , 1039.591], [  197.199 , 1196.644] , [ 458.248 , 677.17 ] , [ 1479.827  , 549.604]])
    ...
    for i in range(numim):
        ...
        if np.isscalar(data_xyexp[0]): continue
        for j in range(4):
...
if 0 :  # matrices a partir des positions de 2 pics de HKL connus
    ...
    xy2 = column_stack((xy4all[:, npic1 * 2:npic1 * 2 + 2], xy4all[:, npic2 * 2:npic2 * 2 + 2]))
    ...
    fileout = filepath + "et1_mat2C_img2828_6362_p" + str(npic1) + "_p" + str(npic2) + ".dat"
...
if 0 :  # desorientation par rapport a la matrice moyenne
    ...
    for k in range(nimg):
        ...
        vec_crystal[k, :], vec_lab[k, :], angle1[k] = twomat_to_rotation(matmean3x3, mat2)
...
def spotlist_gen(Emin, Emax, diagr, matwithlatpar, cryst_struct, showall, \
             calib, pixelsize=0.08056640625, remove_harmonics="yes"):
            ...
            if ((not (K - H) % 2) | (cryst_struct == 'BCC')):
                for L in range(-Hmax, Hmax):
                    if ((not (L - H) % 2) | (cryst_struct == 'BCC')) :
                        if ((cryst_struct == 'FCC') | 
                            ((cryst_struct == 'diamond') & ((H % 2) | ((not H % 2) & (not (H + K + L) % 4)))) | 
                            ...
                            if norme(qlab) > 1.0e-5 :
                                ...
                                if (sintheta > 0.0) & (cosangle2 > cosangle) :
                                    # print "reachable reflection"
                                    Etheor[nspot] = DictLT.E_eV_fois_lambda_nm * norme(qlab) / (2 * sintheta)
                                    ththeor[nspot] = (180.0 / math.pi) * arcsin(sintheta)
                                    # print "Etheor = ", Etheor[nspot]
                                    if (Etheor[nspot] > (Emin * 1000.0)) & (Etheor[nspot] < (Emax * 1000.0)):
    ...
    if remove_harmonics == "yes" :
        hkl2, uflab2, xy2, nspots2, isbadpeak2 = remove_harmonic(hkl[0:nspot, :], uflab[0:nspot, :], xy[0:nspot, :])
        ...
        hkl2, uflab2, xy2, nspots2, Etheor2, ththeor2, chi2, tth2 = hkl[range1], uflab[range1], xy[range1], \
                                    nspot, Etheor[range1], ththeor[range1], chi[range1], tth[range1]
    ...
    if (showall):
        ...
        for i in range(nspots2):
            print(hkl2[i, :], uflab2[i, :].round(decimals=3), \
...
def spotlist_360(Emin, Emax, matwithlatpar, cryst_struct, showall, uilab=array([0., 1., 0.])):
                ...
                for L in range(-Hmax, Hmax):
                    if ((not (L - H) % 2) | (cryst_struct == 'BCC')) :
                        if ((cryst_struct == 'FCC') | 
                            ...
                            # print "hkl =", H,K,L
                            qlab = float(H) * mat[0:3] + float(K) * mat[3:6] + float(L) * mat[6:]
                            if norme(qlab) > 1.0e-5 :  # j'avais oublie de virer HKL = 000
                                uqlab = qlab / norme(qlab)
...
def twofitfiles_to_rotation(filefit1=None, matref1=None, filefit2=None, use_opsym="yes", axis_in_first_stereo_triangle=None):
    ...
    if axis_in_first_stereo_triangle != None :
        matstarlab1, transfmat, rgb_axis = calc_cosines_first_stereo_triangle(matstarlab1, axis_in_first_stereo_triangle)
        matstarlab2, transfmat, rgb_axis = calc_cosines_first_stereo_triangle(matstarlab2, axis_in_first_stereo_triangle)
