...
LIST_TXTPARAM_BS = ['Folder .fit file', 'Folder Result file', 'prefix .fit file',
                    '.fit suffix',
                 'Nbdigits filename', 'startindex', 'finalindex', 'stepindex',
                 'stiffness file (.stf)',
                'nx', 'ny',
                'fast axis',
                'stepxy']
        ...
        for kk, txt_elem in enumerate(txt_fields):
            ...
            if kk in (0, 1, 2, 8):
                ...
                if kk == 0:
                    ...
                    btnbrowse.SetToolTipString('Select output folder for summary results files')
                elif kk == 2:
                    btnbrowse.Bind(wx.EVT_BUTTON, self.OnbtnBrowse_filepathout_fit)
                    btnbrowse.SetToolTipString('Select one .fit file to get (and guess) the generic prefix of all .fit filenames')
                elif kk == 8:
...
#         grid.Add(none10)
        ...
        btn_fabrication_to_hand = wx.Button(self.panel, -1, "Build manually", size=(200, -1))
        grid.Add(btn_fabrication_to_hand)
        btn_fabrication_to_hand.Bind(wx.EVT_BUTTON, self.OnbuildManually)
        btn_fabrication_with_image = wx.Button(self.panel, -1, "Build from Images Info", size=(200, -1))
        btn_fabrication_with_image.Disable()
        grid.Add(btn_fabrication_with_image)
        btn_fabrication_with_image.Bind(wx.EVT_BUTTON, self.Onbtn_fabrication_with_image)
    ...
    def OnbtnBrowse_filepathout_fit(self, evt):
        folder = wx.FileDialog(self, "Select refined Peaklist File .fit for catching prefix",
...
#             print "filename", filename
    ...
    def OnbuildManually(self, event):
        PSboard = Manual_XYZfilecreation_Frame(self, -1,
                                'File sample XYZ position: Manual creation Board')
        PSboard.Show(True)
...
#         modgraph.outfilename = (str(objet_BS.list_valueparamBS[0]) + 'summary_new')
        ...
        if self.builddatfile.GetValue():
            ...
            allres, fullpath_summary_filename = MG.build_summary(image_indices,
                             folderfitfiles,
                            prefix,
                             suffix,
                             filexyz,
            ...
            fullpath_summary_filename = MG.add_columns_to_summary_file_new(fullpath_summary_filename,
...
#         self.Bind(wx.EVT_BUTTON, lambda event: self.Onbtnhelp_ystep(self), id=25)
        ...
        btnStart_BS_fabricationHand = wx.Button(self.panel, -1, "Create File with array Index,X,Y", size=(-1, 60))
        ...
        self.help = wx.TextCtrl(self.panel, -1, '',
                    style=wx.TE_MULTILINE | wx.TE_READONLY, size=(250, 100))
...
#         self.help.SetValue(str(help_a_remplir))
    def Onbtnhelp_xstep(self, event):
        ...
        help_a_remplir += 'By increasing image index position along fast axis increases by Dx.\n'
...
#         modgraph.outfilenamexy = (str(list_valueparam_BS[2]) + 'xy_')
        if check == 1:
            ...
            outfilename = MG.build_xy_list_by_hand(prefix + '_xy_',
                               nx, ny,
                               xfast, yfast,
                               xstep, ystep,
            ...
            wx.MessageBox('Operation Successful! \t \t Filexyz created: %s' % \
                                outfilename)
        else:
...
class New_indimg(wx.Frame):
    def __init__(self, parent, id, title, list_txtctrl_hand):
        ...
        self.Bind(wx.EVT_BUTTON, lambda event: self.Onbtnhelp_nblastpicture(self), id=11)
        ...
        vbox = wx.BoxSizer(wx.VERTICAL)
        vbox.Add(grid, 0, wx.EXPAND)
        self.help = wx.TextCtrl(self.panel, -1, '', style=wx.TE_MULTILINE | wx.TE_READONLY, size=(100, 300))
    ...
    def OnbtnOK(self, event, list_txtctrl_hand) :
        ...
        list_txtctrl_hand[6].SetValue(str('range(' + str(self.list_txtctrl_new[0].GetValue()) + ',' + str(self.list_txtctrl_new[1].GetValue()) + ',' + str(self.list_txtctrl_new[2].GetValue()) + ')'))
...
class SetParameters_BuildSummary_fabricationImage(wx.Frame):
    def __init__(self, parent, id, title, objet_PS, objet_BSi, manag, list_txtctrl):
        wx.Frame.__init__(self, parent, id, title, wx.DefaultPosition, wx.Size(1000, 900))
        ...
        btnStart_BS_fabricationImage.Bind(wx.EVT_BUTTON, lambda event: self.OnbtnStart_BS_fabricationImage(self, manag, objet_BSi, list_txtctrl))
    ...
    def OnbtnStart_BS_fabricationImage(self, event, manag, objet_BSi, list_txtctrl):
        ...
        for k in range(2, 8):
            if k == 5:
                objet_BSi.list_valueparamBSi[k] = int(self.list_txtctrl_image[k].GetValue())
                PAR.number_of_digits_in_image_name = int(self.list_txtctrl_image[k].GetValue())
                list_txtctrl[k].SetValue(str(self.list_txtctrl_image[k].GetValue()))
            elif k != 6:
                objet_BSi.list_valueparamBSi[k] = str(self.list_txtctrl_image[k].GetValue())
            else:
                objet_BSi.list_valueparamBSi[k] = modgraph.indimg
                list_txtctrl[k].SetValue(('range' + '(' + str(modgraph.nbpicture1) + ',' + str(modgraph.nblastpicture) + ',' + str(modgraph.increment) + ')'))
            if objet_BSi.list_valueparamBSi[k] is None or objet_BSi.list_valueparamBSi[k] == 'None' or objet_BSi.list_valueparamBSi[k] == '':
                check = 0
        ...
        if  check == 1:
                objet_BSi.list_valueparamBSi[8] = MG.get_xyzech(objet_BSi.list_valueparamBSi[7], objet_BSi.list_valueparamBSi[3], modgraph.indimg, objet_BSi.list_valueparamBSi[4], modgraph.outfilenamexy)
                list_txtctrl[1].SetValue(str(objet_BSi.list_valueparamBSi[8]))
                wx.MessageBox('Operation Successful! Files created here: %s' % (modgraph.outfilenamexy))
        else:
            wx.MessageBox('Files\'s path or input datas are missing!')
        self.Destroy()
...
def fill_list_valueparamBS(initialparameters):
    ...
    """
    list_valueparam_BS = [initialparameters['IndexRefine PeakList Folder'],
...
#         initialparameters['Detector Calibration File (.dat)'] = MainFolder + 'Gemono/Ge_0005_LT_1.dat'
    ...
    BSFrame = MainFrame_BuildSummary(None, -1, 'Build Summary Parameters Board',
                                     list_valueparamBS)
    BSFrame.Show(True)
