...
try:
    from multigrain import (filter_peaks,
...
class IndexRefineParameters(wx.Frame):
    ...
    def __init__(self, parent, _id, title, listParameters, nb_of_materials=1):
        ...
        btnQuit = wx.Button(self.panel, -1, "Cancel",
                             size=(-1, 50))
...
#             print dir(self.nb)
    ...
    def OnSaveConfigFile(self, evt):
        ...
        file = wx.FileDialog(self, "Save irp File", defaultDir=defaultdir,
                              wildcard=wcd,
        ...
        if file.ShowModal() == wx.ID_OK:
            ...
            outputfilename = ISS.saveIndexRefineConfigFile(self.dict_param_list,
            ...
            for k_page, pagematerial in enumerate(self.materialpages_list):
                for kk, key in enumerate(LIST_TXTPARAMS):
                    if key in list(self.dict_param_list[k_page].keys()):
                        pagematerial.list_valueparamIR[kk] = self.dict_param_list[k_page][key]
    ...
    def hascorrectvalue(self, kk, val):
        ...
        if kk in (1, 11):
            try:
                ...
                wx.MessageBox('Error in Index_Refine.py hascorrectvalue().\nWrong type %s! Must be integer' % self.list_txtparamIR[kk], 'Error')
                flag = False
        ...
        if kk==12:
            ...
            try:
                ...
                wx.MessageBox('Error in Index_Refine.py hascorrectvalue().\nWrong type %s! Must be list of 3 integers' % self.list_txtparamIR[kk], 'Error')
                flag = False
...
class MainFrame_indexrefine(wx.Frame):
    ...
    def __init__(self, parent, id, title, initialparameters, objet_IR):
        wx.Frame.__init__(self, parent, id, title,
                          wx.DefaultPosition, wx.Size(850, 650))
        ...
        for kk, txt_elem in enumerate(objet_IR.list_txtparamIR):
            ...
            if kk in (0, 1, 2, 3, 9, 10, 12):
                ...
                if kk == 0:
                    ...
                    btnbrowse.SetToolTipString('Select one .dat or .cor file to get (and guess) the generic prefix of all peaks list filenames')
                elif kk == 9:
                    btnbrowse.Bind(wx.EVT_BUTTON, self.OnbtnBrowse_filedet)
                    btnbrowse.SetToolTipString('Select detector calibration parameters .det file')
                elif kk == 10:
                    btnbrowse.Bind(wx.EVT_BUTTON, self.OnbtnBrowse_matsfile)
                    btnbrowse.SetToolTipString('Select list of guessed UB matrices or check orientation parameters (.mat,.mats or .ubs) file')
                elif kk == 12:
        ...
        btnStart = wx.Button(self.panel, -1,
                             "START INDEX and REFINE (files .fit in OutPutFolder)")  # ,size=(-1, 50))
        ...
        Createcfgbtn.SetToolTipString('Create .irp file containing parameters to index & refine peaks list')
        self.previousreschk.SetToolTipString('If checked, prior to indexation from scratch (according to .irp file)first if  orientation matrix of image n-1 is a good guess for indexing the current image n')
    ...
    def OnbtnBrowse_filepathout_fit(self, event):
        folder = wx.DirDialog(self,
              "Select folder for indexed and refined peaklist .fit files")
        if folder.ShowModal() == wx.ID_OK:
    ...
    def OnbtnBrowse_filepathout_cor(self, event):
        folder = wx.DirDialog(self,
              "Select folder for peaklist .cor files")
        if folder.ShowModal() == wx.ID_OK:
...
#             referencefiledat_init = self.list_txtctrl[10].GetValue()
            ...
            if referencefiledat_init is not None:
                MAXPIXDEV_CALIBRATIONREFINEMENT = self.initialparameters['maxpixdev_filter_peaks_index_refine_calib']
                self.referencefiledat_purged = filter_peaks(referencefiledat_init,
                               maxpixdev=MAXPIXDEV_CALIBRATIONREFINEMENT)
                (calib_fitfilename,
                npeaks_LT,
                pixdev_LT) = index_refine_calib_one_image(self.referencefiledat_purged,
                                                                filedet=filedet)
            else:
                raise ValueError('filter_peaks_index_refine_calib=1 without .dat file of peaks used for calibration is no more used in Index_refine()')
    ...
    def fitFolderExists(self):
        ...
        if not os.path.isdir(fitfolder):
            try:
                ...
                wx.MessageBox('Can not create %s to contain peaks list .fit files !' \
                              % fitfolder, 'Error')
                return False
    ...
    def corFolderExists(self):
        ...
        if not os.path.isdir(corfolder):
            try:
                ...
                wx.MessageBox('Can not create %s to contain peaks list .cor files !' \
                              % corfolder, 'Error')
                return False
    ...
    def OnStart(self, event):
        ...
        if 0:  # odile's way
            ...
            serial_index_refine_multigrain(filepathdat,
                                          fileprefix,
                                          indimg,
                                          filesuffix,
                                          filefitcalib,
            ...
            serial_index_refine_multigrain_v2(filepathdat,
                                          fileprefix,
                                          indimg,
                                          filesuffix,
                                          filefitcalib,
...
#             print "listfiles", listfiles
            ...
            if nbfiles==0:
                wx.MessageBox('Apparently the folder %s is empty!' % filepathdat,
                                                    'ERROR')
                return
...
#             Index_Refine_Parameters_dict['kf_direction'] = DEFAULT_KF_DIRECTION
            ...
            try:
                ...
                wx.MessageBox('You should enter integer values for images index fields',
                                      'ERROR')
                return
            ...
            if guessedMatricesFile not in ('None','none',) :
                ...
                Index_Refine_Parameters_dict['MinimumMatchingRate'] = 101.
            # ----------------------------------------------------------------------
            ...
            if self.parent is not None:
                ...
                object_to_set.initialparameters['(stepX, stepY) microns'] = (1., 1.)
            ...
            if nb_cpus == 1:
                ...
                wx.MessageBox('Indexation and Refinement not completed.\n Check the prefixfilename of .dat file! Launch the task with only one CPU', 'INFO')
