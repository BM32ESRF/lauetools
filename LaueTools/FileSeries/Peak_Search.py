...
LIST_TXTPARAM_FILE_PS = ['ImageFolder',
                     'OutputFolder',
...
class PeakSearchParameters(wx.Frame):
    def __init__(self, parent, _id, title, listParameters):
        ...
        btnLoad = wx.Button(self.panel, -1, "Load File",
                             size=(-1, 50))
        btnLoad.Bind(wx.EVT_BUTTON, self.OnLoad)

        btnLoadDefault = wx.Button(self.panel, -1, "Reset",
                             size=(-1, 50))
        btnLoadDefault.Bind(wx.EVT_BUTTON, self.OnLoadDefault)

        btnQuit = wx.Button(self.panel, -1, "Cancel && Quit",
                             size=(-1, 50))
    ...
    def hascorrectvalue(self, kk, val):
        ...
        if kk in (0, 5):
            try:
                v = int(val)
                if v not in (0, 1, 2):
                    wx.MessageBox('%s must be equal to 0,1 or 2' % self.list_txtparamIR[kk], 'Error')
                    flag = False
            except ValueError:
...
#                 flag = False
        ...
        # positive float
            ...
            try:
                ...
                wx.MessageBox('wrong type for %s! (kk=%d) Must be float' % (self.list_txtparamIR[kk], kk), 'Error')
                flag = False
    ...
    def OnSave(self, evt):
        ...
        file = wx.FileDialog(self, "Save psp File", defaultDir=defaultdir,
                              wildcard=wcd,
        ...
        if file.ShowModal() == wx.ID_OK:
            ...
            outputfilename = RMCCD.savePeakSearchConfigFile(self.dict_param,
    ...
    def OnLoad(self, evt):
        pspfile = wx.FileDialog(self, "Open Peak Search Parameters",
...
class MainFrame_peaksearch(wx.Frame):
    def __init__(self, parent, _id, title, initialparameters, objet_PS):
        ...
        for kk, txt_elem in enumerate(objet_PS.list_txtparamIR):
            ...
            if kk in (0, 1, 2, 8, 9, 10):
                ...
                if kk == 0:
                    ...
                    btnbrowse.SetToolTipString('Select Folder containing results peaks list .dat files')
                elif kk == 2:
                    btnbrowse.Bind(wx.EVT_BUTTON, self.OnbtnBrowse_CCDfileimage)
                    btnbrowse.SetToolTipString('Select one image file to get (and guess) the generic prefix of all image filenames')
                elif kk == 8:
                    ...
                    btnbrowse.SetToolTipString('Select peak list .dat file to remove peaks from the current peaks list')
                elif kk == 10:
                    btnbrowse.Bind(wx.EVT_BUTTON, self.OnbtnBrowse_pspfile)
                    btnbrowse.SetToolTipString('Select .psp file containing Peak Search parameters to be used on images')

            elif kk == 3:
                self.comboCCD = wx.ComboBox(self.panel, -1, 'MARCCD165',
                                            choices=self.allMaterialsnames, style=wx.CB_READONLY)
        ...
        # tooltips
        btnStart.SetToolTipString('Start Peak Search on all images and create a list of peaks (.dat file) for each image')
        Createcfgbtn.SetToolTipString('Create .psp file containing parameters to peak search & peak position refinement')
...
#         print "listvals", listvals
        ...
        PSPboard = PeakSearchParameters(self, -1, 'PeakSearch Parameters', (LIST_TXTPARAMS,
                                                                            listvals,
                                                                            LIST_UNITSPARAMS))
        PSPboard.Show(True)

    def OnbtnBrowse_filepath(self, event):
...
#             print "filename", filename
    ...
    def OnbtnBrowse_bkgfile(self, event):
    ...
    def OnbtnBrowse_pspfile(self, event):
        pspfile = wx.FileDialog(self, "Select Peak Search Parameters File",
                             wildcard='PeakSearch Param.(*.psp)|*.psp|All files(*)|*')
        if pspfile.ShowModal() == wx.ID_OK:
    ...
    #         import Index_Refine as IR
            ...
            object_to_set.initialparameters['IndexRefine PeakList Folder'] = os.path.join(os.path.dirname(dirname_out),
                                                                'fitfiles')
            object_to_set.initialparameters['PeakListCor Folder'] = os.path.join(os.path.dirname(dirname_out),
                                                                'corfiles')
            object_to_set.initialparameters['PeakList Filename Prefix'] = filenameprefix
            object_to_set.initialparameters['IndexRefine Parameters File'] = os.path.join(os.path.dirname(dirname_out),
                                                                'None')
            object_to_set.initialparameters['Detector Calibration File .det'] = os.path.join(os.path.dirname(dirname_out),
                                                                'None')
            object_to_set.initialparameters['Detector Calibration File (.dat)'] = os.path.join(os.path.dirname(dirname_out),
                                                                'None')
...
#         print "dict_param in file series", dict_param
        if nb_cpus == 1:
            RMCCD.peaksearch_fileseries(fileindexrange, filenameprefix,
                          suffix=suffix, nbdigits=nbdigits,
                          dirname_in=dirname_in,
                          outputname=None, dirname_out=dirname_out,
                            CCDLABEL=self.CCDlabel,
                            KF_DIRECTION='Z>0',
                            dictPeakSearch=dict_param)
        else:
            RMCCD.peaksearch_multiprocessing(fileindexrange, filenameprefix,
                          suffix=suffix, nbdigits=nbdigits,
                          dirname_in=dirname_in,
                          outputname=None, dirname_out=dirname_out,
                            CCDLABEL=self.CCDlabel,
...
initialparameters['PeakSearch Parameters File'] = os.path.join(MainFolder,
                                        'PeakSearch_nanox2_400_0000_LT_4.psp')
...
if __name__ == '__main__':
    ...
    PeakSearchSeries = MainFrame_peaksearch(None, -1,
                                            'Peak Search Parameters Board',
                                            initialparameters,
                                            Stock_PS)
