...
class H5file:
    defaultcolumns = ['start_time', 'end_time',
                      'sample_dataset_scanindex', 'fullcommand',
                                              'scanindex','scantype','motors','localhdf5file','imagefolder', 'endreason']
    ...
    def getscans(self, n_last:int=1000, node_prefix:str=None,  node_substrings: list = None, substrings_logic: str = "OR", only_endreason_SUCCESS:bool=False, verbose:int=0):
        self.listscans, self.listcts = get_scans_cts(self.path, n_last=n_last, node_prefix=node_prefix, node_substrings=node_substrings,substrings_logic=substrings_logic, verbose=verbose-1)
...
class SpecFile:
    ...
    def __init__(self, spec_file, verbose=False, filetype='spec', collectallscans=True,
        ...
        if filetype == 'spec':
            try:
                with open(spec_file,
                        'r') as f:  # the U mode indicates universal line break, essential for accurate counting. # python3.12 anymore rU
    ...
    def get_cmd_list(self, listprops):
        ...
        return { elem[0]: elem[5] for elem in listprops}
...
def getall_from_hdf5file(
    ...
    substrings_logic: str = "OR"  # New parameter: "AND" or "OR"
):
...
def getscans_from_hdf5file(
    ...
    filtering by node name prefix/substrings, and including additional attributes like 'end_reason'.

    This function reads an HDF5 file, filters nodes based on optional criteria (prefix, substrings),
    sorts them by a timestamp attribute (default: 'start_time'), and processes the most recent nodes first.
    It also appends the 'end_reason' attribute (if available) to the properties of each node.

    Args:
        filename (str): Path to the HDF5 file.
        verbose (int, optional): Verbosity level for debugging output. Defaults to 0.
        collectallscans (bool, optional): If True, collect all scans regardless of type. Defaults to True.
        onlywirescan (bool, optional): If True, only collect wire scans. Defaults to False.
        onlymesh (bool, optional): If True, only collect mesh scans. Defaults to False.
        collectall (bool, optional): If True, collect all properties. Defaults to False.
        n_last (int, optional): Number of most recent nodes to process. If None, process all nodes. Defaults to None.
        time_attr (str, optional): Name of the timestamp attribute/dataset to sort by. Defaults to "start_time".
        node_prefix (str, optional): Filter nodes to those starting with this prefix. Defaults to None.
        node_substrings (list, optional): Filter nodes to those containing at least one (OR) or all (AND) of these substrings.
            Defaults to None.
        substrings_logic (str, optional): Logic for substring filtering: "OR" (default) or "AND".

    Returns:
        tuple: Two lists of scan properties:
            - First list: Filtered and sorted scan properties (based on selection criteria).
            - Second list: All scan properties (unfiltered but sorted by date).
            Each scan property list element includes the original properties plus the 'end_reason' attribute (if available).

    Raises:
        ValueError: If `substrings_logic` is not "AND" or "OR".

    Example:
        >>> scans, all_scans = getscans_from_hdf5file(
        ...     "data.h5",
        ...     verbose=1,
        ...     n_last=5,
        ...     node_prefix="align",
        ...     node_substrings=["lig", "BaTiO3"],
        ...     substrings_logic="OR"
        ... )
        >>> for scan in scans:
        ...     print(scan)  # Each scan includes 'end_reason' as the last element
    """
    ...
    with h5py.File(filename, 'r', locking=False) as f:
        # Step 1: Collect all nodes and their start_time, filtered by prefix and substrings
        nodes_with_time = []
        ...
        # Step 2: Sort nodes by start_time (newest first)
        nodes_with_time.sort(key=lambda x: x[1], reverse=True)

        # Step 3: Process nodes in order (newest to oldest)
        listprops = []
        allprops = []
        processed_nodes = 0
        ...
        for name, start_time in nodes_with_time:
            if n_last is not None and processed_nodes >= n_last:
                break  # Stop early if we've processed enough nodes
        ...
        # Sorting by increasing date (if needed)
...
def getscanprops_lowest_hdf5(filename, key, collectallscans=True, onlymesh=False, onlywirescan=False, verbose:int=0, collectall=False):
    ...
    with h5py.File(filename, 'r', locking=False) as f:
        ...
        # Read end_reason from the node (dataset or attribute)
        end_reason = None
        ...
        if onlymesh:
            if any(cmd in scancommand for cmd in ("amesh", 'fscan2d')):
                ...
                props = ['%s_%s' % (keyfilename, idx), idx, postfix, startdate, enddate, '%s_%s %s' % (keyfilename, idx, scancommand), filename, end_reason]
                isselected = True
        elif onlywirescan:
...
def build_dict_scan_from_imagefile(filepath:int, pdf:"PandasDataFrame", CCDLabel:str='sCMOS')->dict:
    blisscommand, imagedate, pdf_idx = IOimage.fromscmosdate2blisscommand(filepath, pdf)
...
def build_dict_scan(item_idx:int, pdf:"PandasDataFrame", CCDLabel:str='sCMOS',verbose:int=0)->dict:
    ...
    for key in ('start_time', 'end_time','sample_dataset_scanindex', 'fullcommand',
                                                  'scanindex','motors','localhdf5file','imagefolder', 'endreason'):
...
def get_scans_cts(pathHDF5, potential_motors=('xech','yech','zech','hfoc','zf','xtech','ytech', 'thf','xps','yps'), potential_scantypes=('ascan','amesh','loopscan','a2scan', 'fscan2d', 'fscan','loopscan'), n_last:int=1000, node_prefix:str=None,  node_substrings: list = None, substrings_logic: str = "OR", only_endreason_SUCCESS=False, verbose:int=0):
    res = getall_from_hdf5file(pathHDF5, n_last=n_last, node_prefix=node_prefix, node_substrings=node_substrings, substrings_logic=substrings_logic, verbose=verbose-1)   # scans and ct
    ...
    for elem in res[0]:
        #print('elem',elem)
        kkey,scanindex,jj,ddate,edate, longcommand, localhdf5file, endreason = elem
        ...
        if scantype in ('ct'):
            listcts.append([ddate,edate, kkey,fullcommand,scanindex,scantype, motors, localhdf5file, imagefolder, endreason])

        if not foundmotors:
            continue
        if scantype in potential_scantypes:
            listscans.append([ddate,edate, kkey,fullcommand,scanindex,scantype, motors, localhdf5file, imagefolder, endreason])
...
# methods of future class  scan (see DAXM)
def getpeaklist(dictparam:dict, data:np.ndarray=None, filepath:str=None):
    """set item peaklist (2D array) of dictparam"""
    if data is not None:
        assert len(data.shape)==2
        dictparam['peaklistfile'] = None
        dictparam['peaklist'] = data
        print("key 'peaklist' is filled")
        return
    
    if filepath is not None:
        dictparam['peaklistfile'] = filepath
    
    if dictparam['peaklistfile'].endswith('dat'):
        # read file in dict.peaklistfile
        data=IOLT.read_Peaklist(dictparam['peaklistfile'])[:,:2]
    else:
        XY = IOLT.readfile_cor(dictparam['peaklistfile'])[3:5]
        data =np.array(XY).T
    
    print('getpeaklist data', data)    
            
    dictparam['peaklist'] = data
    print(" read 'peaklistfile' and set 'peaklist'")




def sortpeaklist(dictparam):
    """sort peaklist by increasing y ---for multiprocessing purpose (pixel values)------"""
    dictparam['peaklist'] = GT.sortpeaklist_by_y(dictparam['peaklist'])
    print('peaks are sorted by Y position')

def setnumberimagesperline(dictparam, nbimagesperline=None):
    """ set dictparam['nbimagesperline'] """
    if nbimagesperline is not None:
        assert isinstance(nbimagesperline, int)
        dictparam['nbimagesperline']=nbimagesperline
    elif 'mapdimensions' in dictparam:
        mapdim=dictparam['mapdimensions']
        if len(mapdim)==2:
            dictparam['nbimagesperline']=mapdim[1]
        else:
            dictparam['nbimagesperline']=mapdim[0]
