...
def getUBs_and_MatchingRate(spot_index_1, spot_index_2, ang_tol_LUT, angdist, coords_1, coords_2,
    ...
    (list_orient_matrix, planes, pairspots) = UBs_from_twospotsdistance(spot_index_1,
                                                                        spot_index_2,
                                                                        ang_tol_LUT,
                                                                        angdist,
                                                                        coords_1,
...
def UBs_from_twospotsdistance(spot_index_1,
    ...
    # when hkl1 is guessed (and set)
        ...
        (hkls, _), LUTspecific = FindO.PlanePairs_from2sets(exp_angular_dist, angle_tol,
                                                set_hkl_1, hkl2,
...
def Loop_on_PlanesPairs_and_Get_Matrices(PP_list, spot_index, coord1, coords, B,
    ...
    for k, PP in enumerate(PP_list):
        ...
        print("**Looking up for the %d planes pairs in LUT from exp. spots "
        ...
        for planepair in hkls:
            ...
            if np.sum(np.abs(np.cross(hkl1,hkl2)))==0.:
    ...
    for spotindex_2, angle in enumerate(Distances_from_central_spot):
        ...
        if hkls is not None and (spot_index != spotindex_2):
            ...
            if nbpairs == 1:
                ...
                (allres, resclose, nbclose, nballres,
                    mean_residue,
                    max_residue,
                ) = AngRes
