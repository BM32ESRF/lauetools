...
def matrices_from_onespot_hkl(
    ...
    for spotindex_2, query_angle in enumerate(Distances_from_central_spot):
        ...
        hkls, LUT = FindO.PlanePairs_from2sets(query_angle,
                                            LUT_tol_angle,
                                            hkl1,
                                            hkl2,
                                            key_material,
...
def getUBs_and_MatchingRate(
    ...
    if len(BestScores_per_centralspot) > 0:
        ...
        print(
            "No orientation matrix found with nb of matches larger than the given threshold 'Minimum_Nb_Matches' = %d"
            % Minimum_Nb_Matches
        )
...
def Loop_on_PlanesPairs_and_Get_Matrices(PP_list, spot_index, coord1, coords, B,
    ...
    for k, PP in enumerate(PP_list):
        ...
        print("**Looking up for the %d planes pairs in LUT from exp. spots (%d, %d): " % (nbplanepairs, spot_index, spotindex_2))
...
def getOrientMatrices(spot_index_central,
    ...
    for k_centspot_index, spot_index_central in enumerate(list_spot_central_indices):
        ...
        if hkl is not None:
            ...
            hkl2, 
            LUTspecific,
            ) = matrices_from_onespot_hkl(spot_index_central,
                                        LUT_tol_angle,
                                        Tab_angl_dist,
                                        twiceTheta_exp,
                                        Chi_exp,
        ...
        if len(BestScores_per_centralspot[k_centspot_index]) > 0:
            ...
            # sort results
            rank = np.lexsort(
                keys=(BestScores_per_centralspot[k_centspot_index][:, 2],
...
def build_AnglesLUT_fromlatticeparameters(latticeparameters, n,
    ...
    if MaxRadiusHKL not in (False, 0, 0.0):
        ...
        print('nb of nodes after resolution filtering %d\n'%len(Hf))
