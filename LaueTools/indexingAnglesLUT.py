...
def Possible_planes(angles_value, tole:deg=0.2, verbose=1, onlyclosest=1):
    """
...
def plane_type_attribution(spot_index_1:int, angulartolerance:deg, table_angdist):
    """
...
def plane_type_attribution_twospots(spot_index_1:int, spot_index_2:int, angulartolerance:deg, table_angdist):
    """
...
def twospots_recognition(spot_1:Iterable[float], spot_2:Iterable[float], angulartolerance:deg, onlyclosest=1):
    """
    ...
    """
    if len(spot_1)!=len(spot_2):
        raise ValueError(f'{spot_1} and {spot_2} should have 2 elements each')
...
def matrices_from_onespot(spot_index:int, ang_tol:deg, table_angdist, Theta, Chi, verbose:int=0):
    """
...
def matrices_from_onespot_hkl(spot_index:int, LUT_tol_angle:deg, table_angdist, twiceTheta_exp, Chi_exp,
    ...
    if allow_restrictedLUT:
        ...
        allow_restrictedLUT = CP.isCubic(latticeparams) or CP.isHexagonal()(latticeparams)
...
def matrices_from_onespot_new(spot_index, ang_tol, table_angdist, twiceTheta, Chi, n, B0,
    ...
    return Loop_on_PlanesPairs_and_Get_Matrices(PPs_list, spot_index, coord_central_spot,
                                            coords_exp, B0, verbose=verbose-1,
                                            excludespotspairs=excludespotspairs)
...
def UBs_from_twospotsdistance(spot_index_1, spot_index_2, angle_tol, exp_angular_dist,
    ...
    :param set_hkl_1     : if not None, [h,k,l] of spot #1
...
def Loop_on_PlanesPairs_and_Get_Matrices(PP_list, spot_index, coord1, coords, B,
    ...
    and compute corresponding matrices
    ...
    if PP_list == []:
        if verbose>0 : print('PP_list is empty')
        return matrix_list, pairplanes, pairspots
    ...
    for k, PP in enumerate(PP_list):
        ...
        for planepair in hkls:
            ...
            if np.sum(np.abs(np.cross(hkl1, hkl2))) < 0.00000001:
                if verbose>0: print("parallel planes ...")
                continue
...
def getOrientMatrix_from_onespot(spot_index,
    ...
    if MatchingThresholdStop in (None, 0.0, 100.0, 0, 100):
        ...
        res_onespot = getOrientMatrices(spot_index, emax, table_angdist, twiceTheta / 2.0, Chi,
                                        n=n, B=B, cubicSymmetry=cubicSymmetry,
                                        LUT=LUT,
                                        ResolutionAngstrom=ResolutionAngstrom,
...
def getOrientMatrices_SubSpotsSets(selectedspots_ind, emax, Theta_exp, Chi_exp, nLUT,
    ...
    for i1, i2 in GT.allpairs_in_set(range(len(selectedspots_ind))):
        ...
        # (List_UBs, List_Scores), LUT
        UBS_MRS, _ = getUBs_and_MatchingRate(spot_index_1,
                                            spot_index_2,
                                            LUT_tol_angle,
                                            expdistance_2spots,
                                            coords_1,
...
def getOrientMatrices(spot_index_central: Union[Iterable[int], int],
    ...
    if isinstance(spot_index_central, (list, np.ndarray)):
        ...
        if verbose>0: print("looking from spot # 0 (default settings)")
    ...
    if key_material == "":
        raise ValueError(f"Warning! The material {key_material} is not defined in getOrientMatrices()")
    ...
    if set_central_spots_hkl not in (None, "None"):
        ...
        if set_central_spots_hkl.shape == (3,):
            if verbosedetails>0: print("case: 1a")
            ...
            if verbosedetails>0: print("case: 1b")
        else:
            if verbosedetails>0: print("case: 2")
    ...
    if addMatrix is not None:
        ...
        if verbose>0: print("loop over added matrices")
...
def build_AnglesLUT(B0matrix, n, MaxRadiusHKL=False, cubicSymmetry=False, applyExtinctionRules=None, verbose=0):
    """
    ...
    return build_AnglesLUT_fromlatticeparameters(latticeparameters, n,
                                                MaxRadiusHKL,
                                                cubicSymmetry,
                                                applyExtinctionRules, verbose=verbose)
...
def build_AnglesLUT_fromlatticeparameters(latticeparameters, n,
    ...
    if MaxRadiusHKL not in (False, 0, 0.0):
        ...
        if verbose>1: print('nb of nodes', len(xs))
        ...
        if verbose>1: print('nb of nodes after resolution filtering %d\n'%len(Hf))
...
def getOrients_AnglesLUT(spot_index_central,
    ...
    .. note: used by indexingspotsSet.py
    ...
    if isinstance(spot_index_central, (list, np.ndarray)):
        ...
        if verbose>0: print("looking from spot # 0")
        scan_over_index = [0]
