...
class DetectorParameters(wx.Dialog):
    ...
    def __init__(self, parent, _id, title, Parameters_dict):
        ...
        self.paramdetector = ['Distance', 'xcen', 'ycen', 'betangle', 'gammaangle',
                              'pixelsize', 'dim1', 'dim2', "detectordiameter", "kf_direction"]
        self.units = ['mm', 'pixel', 'pixel', 'deg', 'deg', 'mm', 'pixel', 'pixel', "mm", "ascii"]
        ...
        for kk, paramVal in enumerate(self.params_values_list):
            ...
            self.controltext.append(wx.TextCtrl(self.panel, -1, str(paramVal),
                                                (150, 40 + 30 * kk),
                                                (150, -1)))
        ...
        com.SetFont(font3)
        self.comments = wx.TextCtrl(self.panel, style=wx.TE_MULTILINE, size=(590, sizey), pos=(5, 360))
...
#        self.Centre()
    ...
    def OnSaveCalib(self, event):
        ...
        dlg = wx.TextEntryDialog(self, 'Enter Calibration File name : \n Current Calibration parameters are: \n' + \
                                    txt, 'Saving Calibration Parameters Entry')
        dlg.SetValue('*.det')
        if dlg.ShowModal() == wx.ID_OK:
            ...
            text = "%.17f,%.17f,%.17f,%.17f,%.17f,%.17f,%.17f,%.17f\n" % (dd, xcen, ycen, xbet, xgam, pixsize, dim1, dim2)
        ...
        for ii, controller in enumerate(self.controltext):
            if ii < 9:
                ...
                if val_kf_direction not in DictLT.DICT_LAUE_GEOMETRIES:
                    wx.MessageBox('Value of kf_direction (Laue Geometry) is unknown.\nMust be in %s' % str(list(DictLT.DICT_LAUE_GEOMETRIES.keys())), 'Error')
                    return False
                else:
...
if __name__ == '__main__':
    ...
    Parameters_dict['pixelsize'] = 165. / 2048
    Parameters_dict['framedim'] = (2048, 8956)
    Parameters_dict['detectordiameter'] = 165.
    Parameters_dict['kf_direction'] = 'Z>0'


    DetectorParamGUIApp = wx.App()
    DetectorParamGUIFrame = DetectorParameters(None, -1, "Detector Calibration Board", Parameters_dict)
