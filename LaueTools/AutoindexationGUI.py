...
#         print "self.current_exp_spot_index_list", self.current_exp_spot_index_list
        ...
        print("keys of self.IndexationParameters in DistanceScreeningIndexationBoard",list(self.IndexationParameters.keys()))
    ...
    def initGUI(self):
        ...
        self.nbspotmaxformatching = wx.SpinCtrl(self.panel, -1, str(nbspots_in_data), (600, 110), (60, -1), min=3, max=nbspots_in_data)
        ...
        self.refresh.SetToolTipString('Refresh the Material list to admit new materials built in other LaueToolsGUI menus.')
        ...
        self.showplotBox.SetToolTipString('Plot all exp. and theo. Laue Patterns for which the number of matched spots is larger than "Minimum Number Matched spots".')

        self.filterMatrix.SetToolTipString('Keep only one orientation matrix for matrices which are equivalent (cubic symmetry unit cell vectors permutations).')
    ...
    def SetMaterialsCombo(self, evt):
        ...
        self.combokeymaterial = wx.ComboBox(self.panel, 4, 'Ge',
                                    (220, 70), size=(150,-1), choices=self.list_materials,
...
#                             'Spots Order Reference File']
        ...
        List_Ctrls = [self.combokeymaterial, 1, 5., self.emax, 100.,
                      self.DRTA, MatchingAngleTol, self.nbspotmax, 6,
                      self.spotlist, self.ResolutionAngstromctrl,
                      self.nLUT, sethklcentral, True, nbSpotsToIndex,
                      [MatchingAngleTol, MatchingAngleTol / 2.],
                      self.spotsorder]
...
#                 break
    ...
    def Save_irp_configfile(self, outputfile='mytest_irp.irp'):
        ...
        """
        ISS.saveIndexRefineConfigFile(self.dict_param_list,
...
#             Tabledistance = angulardist.calculdist_from_2thetachi(lc, lc)
        ...
        if wrongsets is True:
            wx.MessageBox('Central spots indices must be strictly lower than the size of Recognition spots set', 'Error')
...
#         print "Matching tolerance angle ", fine_tolangle
        ...
        restrictLUT_cubicSymmetry = (restrictLUT_cubicSymmetry and \
                                     CP.hasCubicSymmetry(self.key_material))
...
#         return
        ...
        if nb_solutions > 1:
            ...
            self.bestmatrices, stats_res = ISS.MergeSortand_RemoveDuplicates(self.bestmatrices,
                                                           stats_res,
                                                           Minimum_MatchesNb,
                                                           tol=0.0001,
        ...
        if nb_solutions:
            ...
            if nb_central_spots == 1:
                ...
                for k_solution in range(nb_solutions):
                    ...
                    TwicethetaChi = LT.SimulateResult(grain, 5, float(self.emax.GetValue()),
                                                                  simulparameters,
                                                                  ResolutionAngstrom=ResolutionAngstrom,
                                                                  fastcompute=1)
                    ...
                    if self.showplotBox.GetValue():
                        ...
                        plotresult = Plot_RefineFrame(self, -1, title,
                                    data=self.data,
                                    datatype='2thetachi',
            ...
            # many central points were used for distance recognition
                ...
                for m in range(nb_to_plot):
                    ...
                    TwicethetaChi = LT.SimulateResult(grain, 5, emax,
                                                      simulparameters,
                                                      ResolutionAngstrom=ResolutionAngstrom,
                                                      fastcompute=1)
            ...
            self.config_irp_filename = self.config_irp_filename[:pos_] + '_%d.irp' % self.indexation_index
...
class RecognitionResultCheckBox(wx.Frame):
    ...
    """
    def __init__(self, parent, _id, title, stats_residues, data, DRTA, MATR,
                 key_material='Ge', emax=25,
                 ResolutionAngstrom=False,
                 kf_direction='Z>0',
                 datatype='2thetachi',
...
#         print "data_XY_exp", self.data_XY_exp
        ...
        self.DataSet = DataSetObject
        print("self.DataSet.detectordiameter in init RecognitionResultCheckBox", self.DataSet.detectordiameter)
    ...
    def init_GUI(self):
        ...
        panel = wx.Panel(self, -1, size=(400, 50 + 20 * self.nbPotentialSolutions + 120))
        ...
        for k in range(self.nbPotentialSolutions):
            self.cb.append(wx.CheckBox(panel, -1,
                                    '     ' + str(k) + 
                                    '                      %d                                 %d                                   %.3f'
                                    % tuple(self.stats_residues[k][:3]), (10, 35 + 20 * k)
                                    )
                        )
        ...
        wx.StaticText(panel, -1, 'Energy min: ', (15, 35 + 20 * self.nbPotentialSolutions + 30))
        self.SCmin = wx.SpinCtrl(panel, -1, '5', (95, 35 + 20 * self.nbPotentialSolutions + 30),
                                 (60, -1), min=5, max=150)

        wx.StaticText(panel, -1, 'Energy max: ', (180, 35 + 20 * self.nbPotentialSolutions + 30))
        self.SCmax = wx.SpinCtrl(panel, -1, str(int(self.emax)), (260, 35 + 20 * self.nbPotentialSolutions + 30),
                                 (60, -1), min=6, max=150)
    ...
    def OnSimulate_S3(self, event):
        ...
        self.toshow = []
        for k in range(self.nbPotentialSolutions):
            if self.cb[k].GetValue() == True: self.toshow.append(k)
        ...
        parentGrainPlot = Plot_RefineFrame(self, -1, "parent grain matrix #%d" % parent_matrix_index,
                                data=self.data,
                                datatype=self.datatype,
                                data_XY=self.data_XY_exp,
                                ImageArray=self.ImageArray,
        ...
        for k_matsigma, vecteurref in enumerate(listmatsigma):
            ...
            grain = [vecteurref, [1., 1., 1.], parent_grain_matrix, element]
...
#            wxmpl.EVT_POINT(plotsigma, plotsigma.plotPanel.GetId(), plotsigma._on_point_choice)
        ...
        RRCB = RecognitionResultCheckBox(self, -1,
                                         'Potential Solutions from Sigma3 Simulations',
                                         res_sigma,
                                         self.data, self.DRTA, self.matr_ctrl,
                                         key_material=self.key_material,
                                         emax=emax,
                                         ResolutionAngstrom=self.ResolutionAngstrom,
                                         kf_direction=self.kf_direction,
...
if __name__ == '__main__':
    ...
    (Current_peak_data,
     ...
     data_pixX, data_pixY,
     data_I,
     calib, CCDCalibDict) = IOLT.readfile_cor(filename,
    ...
    indexation_parameters['detectordiameter'] = 165.
    indexation_parameters['pixelsize'] = 165. / 2048
    ...
    AIGUIApp = wx.App()
    AIGUIframe = DistanceScreeningIndexationBoard(None, -1,
                                          indexation_parameters,
