...
if sys.version_info.major == 3:
    ...
    from GUI.PlotRefineGUI import IntensityScaleBoard
...
class SimulationPlotFrame(wx.Frame):
    ...
    """
    def __init__(self, parent, _id, title, data=(1, 1, 1, 1, 1, "2thetachi", None),
                                                                    ImageArray=None,
                                                                    StreakingData=None,
            ...
            if self.pick_distance_mode:
                ...
                if self.nbclick_dist == 2:
                    ...
                    if self.datatype == "pixels":
                        ...
                        _dist = np.sqrt((spot1[0] - spot2[0]) ** 2 + (spot1[1] - spot2[1]) ** 2)
                    ...
                    if self.datatype == "2thetachi":
                        ...
                        self.txtctrldistance_angle.SetLabel("==> %s deg" % str(np.round(_dist, decimals=3)))
                    if self.datatype == "pixels":
                        print("RES =", _dist)
                        ...
                        self.txtctrldistance_pixel.SetLabel("==> %s pixels" % str(np.round(_dist, decimals=3)))
    ...
    def _replot(self):
        ...
        if self.showFluoDetectorFrame:
            ...
            # Ydetarrow
            x1b, y1b = (center_detframe[0], center_detframe[1])  # base
            x1t, y1t = (pt_lb_prime[0], pt_lb_prime[1])  # tip

            # Zdetarrow
            #x2b, y2b = (center_detframe[0], center_detframe[1])  # base
            x2t, y2t = (pt_zaxis_prime[0], pt_zaxis_prime[1])  # tip
            
            arrows = [patches.FancyArrow(x1b, y1b, x1t - x1b, y1t - y1b, fc="g",
                            width=0.3, head_width=0.9, alpha=0.5),
                    patches.FancyArrow(x1b, y1b, x2t - x1b,y2t - y1b, fc="r",
                            width=0.3, head_width=0.9, alpha=0.5)]
        ...
        if self.experimentaldata:  # plot experimental data
            self.axes.scatter(
                ...
                s=self.func_size_intensity(np.array(self.experimentaldata[2]), 0.02, 0.0, lin=1),
                                                facecolor="r",
                                                edgecolor="r",
                                                marker="+")
    ...
    def drawAnnote(self, axis, x, y, annote):
        ...
        if (x, y) in self.drawnAnnotations:
            ...
            m = axis.scatter([x], [y], s=1, marker="d", c="r", zorder=100, edgecolors="None",
                alpha=0.5)  # matplotlib 0.99.1.1
    ...
    def onSetImageScale(self, _):
        ...
        """
        if self.datatype == "2thetachi":
            wx.MessageBox('Please, open a new simulation window by restarting the simulation '
                                        'with XYPixel "in Display parameters" checked', 'Info')
            return
