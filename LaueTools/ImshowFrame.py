...
class ImshowFrame(wx.Frame):
    ...
    def __init__(self, parent, _id, title, dataarray,
                 Size=(4, 3), center=(100, 100),
                 boxsize=(30, 30), logscale=0,
                 fitfunc=None, fitresults=None,
                 **kwds):
...
#        self.axes.set_autoscale_on(False) # Otherwise, infinite loop
        ...
        if self.fitfunc is not None:
            # print "ya quelqun? --------------"
            self.axes.contour(self.fitfunc(*np.indices(self.data.shape)), cmap=GT.COPPER)
...
class ImshowFrame_old(wx.Frame):
    """ Class to show 2D array intensity data

    """
    def __init__(self, parent, id, title, dataarray,
                 Size=(4, 3), center=(100, 100),
                 boxsize=(30, 30),
                 fitfunc=None, fitresults=None,
                 **kwds):
    ...
    def _replot(self):
        ...
        if self.fitfunc is not None:  # plotting fitting function
            # print "ya quelqun? --------------"
            self.axes.contour(self.fitfunc(*np.indices(self.data.shape)), cmap=GT.COPPER)
            # self.axes.scatter(array([5]),array([5]),marker='x',edgecolor='r') # simply to play with data
        ...
        if self.fitresults is not None:
            # print "ya quelqun  2? --------------"
            # print [self.title]+self.fitresults.tolist()

            self.axes.set_title('%s\n Intbkg= %.1f Int-Intbkg=%.1f \n(X,Y)XMAS=(%.2f,%.2f) (std1,std2)=(%.3f,%.3f) rotAng=%.1f' % tuple([self.title] + self.fitresults.tolist()))
        else:
            self.axes.set_title('%s\n' % self.title)
        self.axes.grid(True)
        ...
        def format_coord(x, y):
            ...
            if col >= 0 and col < numcols and row >= 0 and row < numrows:
                z = self.data[row, col]
                expz = np.exp(z)
                return 'x=%1.4f, y=%1.4f, z=%1.4f,exp(z)=%5.1f' % (x + self.center[0] - self.boxsize[0], y + self.center[1] - self.boxsize[1], z, expz)
            else:
...
if __name__ == '__main__':
    ...
    PSGUIApp = wx.App()
    PSGUIframe = ImshowFrame(None, -1, title, dataarray,
                 Size=(4, 3), center=(150, 150),
                 boxsize=(150, 150),
                 fitfunc=None, fitresults=None,
                 **kwds)
    PSGUIframe.Show()
