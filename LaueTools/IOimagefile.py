...
def fromscmosdate2datetime(imginfo:str, isoformat:bool=False):
    """get date time from sCMOS binary image header written by BLISS on BM32 ESRF
    
    imginfo comes from  read_scmos_Artisttag()

    returns datetime object (isoformat is False) or string of date in isoformat (else)
    """
    datefield = imginfo[1]
    
    strimagedate = datefield[5:].strip()
    rr = strimagedate.replace(' ',':')[:-1]
    jj = map(float,rr.split(':'))
    kk = map(math.floor,jj)
    
    dd = datetime.datetime(*kk)
    if isoformat:
        dd=dd.isoformat()
    return dd

def fromscmosdate2blisscommand(fullpathimage:str, pdf:'PandasDataFrame', returndataframe:bool=False)->str:
    """get BLISS scan command from date of image.tiff and hdf5 panda dataframe object
    
    return:  BLISS command,  image date"""

    try:
        imginfo = read_scmos_Artisttag(fullpathimage)
    except FileNotFoundError:
        print('File not found !!')
        return
    
    querydate = fromscmosdate2datetime(imginfo)
    mask = (pdf['end_time'] > querydate.isoformat()) & (pdf['start_time'] <= querydate.isoformat())
    dfallscansgooddate = pdf.loc[mask]

    # corresponding to bliss command
    blisscommand = 'not found'
    if len(dfallscansgooddate) ==1:
        blisscommand = dfallscansgooddate.values[0][3]
        dfallscans_index = int(dfallscansgooddate.index[0])
    else:
        dfallscans_index = None
                                           
    
    if returndataframe:
        return blisscommand, querydate, dfallscans_index, dfallscansgooddate
    else:
        return blisscommand, querydate, dfallscans_index
...
def readCCDimage(filename, CCDLabel="MARCCD165", dirname=None, stackimageindex=-1, verbose=0):
    ...
    :param stackimageindex: index of image in stacked images, defaults to -1
...
def readrectangle_in_image(filename:str, pixx:int, pixy:int, halfboxx:oddinteger, halfboxy:oddinteger, dirname=None,
...
TEMPLATE_H5_EIGER4MCDTE = "/data/visitor/blc17179/bm32/20260617/RAW_DATA/Ge/Ge_0001/scan0002/eiger4m_0000.h5"
def writeimage_singleh5(filename, data, templateh5filename=None, verbose=0):
    """for eiger 4m CdTe detector, single file=single array"""
    if templateh5filename is None:
        templateh5filename = TEMPLATE_H5_EIGER4MCDTE
    
    if verbose > 0: print("data.shape", data.shape)
    if len(data.shape) == 2:
        data = data.reshape((1, data.shape[0], data.shape[1]))
    
    update_eiger_template(templateh5filename, filename, data)

    if verbose > 0: print("writeimage_singleh5 done")
    

def update_eiger_template(
    template_filename,
    output_filename,
    newdataarray,
    ...
    """
    Create a new HDF5 file from a template by replacing only the detector data.

    The template structure, metadata, links, compression, chunking and
    attributes are preserved exactly. Only the dataset

        /entry_0000/CRGIF/eiger4m/data

    is modified.

    Since
        /entry_0000/measurement/data
        /entry_0000/CRGIF/eiger4m/plot/data

    are soft links, they automatically point to the updated data.
    """

    if not os.path.exists(template_filename):
        raise FileNotFoundError(template_filename)
    ...
    dataset_path = "/entry_0000/CRGIF/eiger4m/data"
    ...
    with h5py.File(output_filename, "r+") as f:
        ...
        # Check compatibility
        # -----------------------------
        if dset.shape != newdataarray.shape:
            raise ValueError(
                f"Shape mismatch.\n"
                f"Template : {dset.shape}\n"
                f"New data : {newdataarray.shape}"
            )

        if dset.dtype != newdataarray.dtype:
            newdataarray = newdataarray.astype(dset.dtype)
        ...
        if "processing_info" in entry:
            del entry["processing_info"]

        proc = entry.create_group("processing_info")

        proc.create_dataset(
            "description",
            data=np.bytes_(processing_description)
        )

        proc.create_dataset(
            "date",
            data=np.bytes_(processing_date)
        )

        proc.create_dataset(
            "method",
            data=np.bytes_(processing_method)
        )

    print(f"Template size : {os.path.getsize(template_filename)/1024**2:.2f} MB")
    print(f"Output size   : {os.path.getsize(output_filename)/1024**2:.2f} MB")
...
def copy_template_and_replace_single_data(
    template_filename,
    output_filename,
    newdataarray,
    processing_description="",
    ...
    """
    Copies the entire structure of the template HDF5 file, replaces only the first '/data' dataset with newdataarray,
    preserves group types, attributes, compression settings, and adds a /entry_0000/processing_info group for metadata.
    """
    if not os.path.exists(template_filename):
        raise FileNotFoundError(f"Template file '{template_filename}' not found.")

    with h5py.File(template_filename, 'r') as template:
        # Find all '/data' datasets in the template
        data_datasets = []
        def find_data_datasets(group, path=""):
            for key in group.keys():
                obj = group[key]
                if isinstance(obj, h5py.Dataset) and key == 'data':
                    data_datasets.append((path + '/' + key, obj))
                elif isinstance(obj, h5py.Group):
                    find_data_datasets(obj, path + '/' + key)

        find_data_datasets(template)

        if not data_datasets:
            raise ValueError("No '/data' datasets found in the template.")

        # Check shape and dtype of newdataarray against the first '/data' dataset
        first_data_path, first_data_dataset = data_datasets[0]
        if newdataarray.shape != first_data_dataset.shape:
            raise ValueError(
                f"Shape mismatch: newdataarray has shape {newdataarray.shape}, "
                f"but template's {first_data_path} has shape {first_data_dataset.shape}."
            )
        if newdataarray.dtype != first_data_dataset.dtype:
            raise ValueError(
                f"Dtype mismatch: newdataarray has dtype {newdataarray.dtype}, "
                f"but template's {first_data_path} has dtype {first_data_dataset.dtype}."
            )

        # Get compression and chunking settings from the first '/data' dataset
        compression = first_data_dataset.compression
        chunks = first_data_dataset.chunks

        def copy_group_with_attributes(source, target):
            # Copy group attributes
            for attr_name in source.attrs:
                target.attrs[attr_name] = source.attrs[attr_name]

            for key in source.keys():
                obj = source[key]
                if isinstance(obj, h5py.Group):
                    new_group = target.create_group(key)
                    copy_group_with_attributes(obj, new_group)
                else:
                    # Replace only the first '/data' dataset
                    if key == 'data' and len(data_datasets) > 0:
                        ...
                        data_datasets.pop(0)  # Remove the first dataset to avoid replacing others
                    else:
                        # Skip other '/data' datasets or copy as-is
                        if key != 'data' or len(data_datasets) == 0:
                            target.create_dataset(
                                key,
                                data=obj[()],
        ...
        with h5py.File(output_filename, 'w') as new_file:
            # Copy the entire structure from the template, preserving group attributes
            copy_group_with_attributes(template, new_file)

            # Add processing_info group under entry_0000
            if 'entry_0000' in new_file:
                processing_info = new_file['entry_0000'].create_group('processing_info')
                processing_info.create_dataset('description', data=processing_description.encode())
                processing_info.create_dataset('date', data=processing_date.encode())
                processing_info.create_dataset('method', data=processing_method.encode())
            else:
                raise ValueError("Template does not contain an 'entry_0000' group.")

    # Check the size of the output file
    template_size = os.path.getsize(template_filename)
    output_size = os.path.getsize(output_filename)
    print(f"Template file size: {template_size / (1024 ** 2):.2f} MB")
    print(f"Output file size: {output_size / (1024 ** 2):.2f} MB")

    print(f"New HDF5 file '{output_filename}' created successfully with your data and processing info.")
