...
#        wx.Dialog.__init__(self, parent, _id, title, size=(1100, 500), style=wx.OK)
        ...
        wx.lib.mixins.listctrl.ColumnSorterMixin.__init__(self.listcontrol,
                                                          len(self.field_name) + 1)
        ...
        for k, elem in enumerate([stf1, stf2, stf3]):
            ...
            vbox3.Add(wx.StaticText(pnl1, -1, 'selected property'),
...
#         self.tcnb.Disable()
        ...
        vbox4.Add(wx.Button(pnl2, wx.ID_OK, 'Accept and Quit', size=(-1, 60)), 0, wx.EXPAND | wx.ALL, 5)
        ...
        for k, condition in enumerate(cond):
            if condition != '':
                ...
                try:
                    ...
                    wx.MessageBox('Wrong condition:\nType simply in field: >5, <=58.5 ==6.' , 'INFO')
                    return
...
#         self.tcnb.SetValue(str(filternbspots))
    ...
    def OnPlot(self, event):
        ...
        bibins = np.linspace(np.amin(columdata), np.max(columdata), max(10, len(columdata) / 10))
        databinned = np.histogram(columdata, bins=bibins)  # heights, bins
        print(("databinned", databinned))
        barplt = BarPlotFrame(self, -1, 'Plot Frequency', databinned, title_in_plot='last column')
        barplt.Show(True)
...
class MyListCtrl(wx.ListCtrl, wx.lib.mixins.listctrl.ColumnSorterMixin):
    ...
    def add_row(self, row, dict_data):
        ...
        if WXPYTHON4:
            index = self.InsertItem(sys.maxsize, str(dict_data[self.field_name[0]][row]))  # ,    self.img_list[content.nature])
            _content = []
    
            for k, field in enumerate(self.field_name):  # loop over other fields to fill
            ...
            index = self.InsertStringItem(sys.maxsize, str(dict_data[self.field_name[0]][row]))  # ,    self.img_list[content.nature])
            _content = []
...
#        self.axes.set_autoscale_on(False) # Otherwise, infinite loop
        ...
        self.axes.bar(leftposition[:-1], heights, width=leftposition[1] - leftposition[0])
...
if __name__ == "__main__":
    ...
    # test data:
    mySpotData = { 'spot index':[0, 1, 3, 6, 9, 10, 2, 5, 4, 11, 20],
                    'Energy' : [8.2, 12.2, 6.3, 13.6, 5., 15., 19., 6.325, 4.58, 9.8796, 10.2154],
                    ...
                    'Intensity': [10000., 558.235, 24.3265, -1.123665, 65535, 100, 1, 0, 445, 2048, 6525.356]}
