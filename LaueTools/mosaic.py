...
class ImshowFrameNew(wx.Frame):
    ...
    """
    def __init__(self, parent, _id, title, dataarray,
...
#         self.fig.canvas.mpl_connect('motion_notify_event', self.onMotion_ToolTip)
        ...
        self.slidertxt_min = wx.StaticText(self.panel, -1,
            "Min :")
        ...
        self.slidertxt_max = wx.StaticText(self.panel, -1,
            "Max :")
        ...
        self.comboLUT = wx.ComboBox(self.panel, -1,
                                    self.LastLUT,
        ...
        self.comboscale = wx.ComboBox(self, -1,
                                    self.scaletype,
                                    choices=['Linear', 'Log'],
                                    size=(-1, 40))
...
#         colorVal = scalarMap.to_rgba(values[idx])
    ...
    def clear_axes_create_imshow(self):
        ...
        self.myplot = self.axes.imshow(self.data_to_Display, cmap=GT.ORRD,
        ...
        def format_coord(x, y):
            ...
            if col >= 0 and col < numcols and row >= 0 and row < numrows:
                ...
                if self.dataarray_info is None:
                    sentence = 'x=%1.4f, y=%1.4f, color=%s, ImageIndex: %d' % \
                                        (x, y, str(z), Imageindex)
                else:
                    sentence = 'x=%1.4f, y=%1.4f, val=%s, ImageIndex: %d' % \
                                        (x, y, self.dataarray_info[row, col], Imageindex)
...
#         self.fig.canvas.mpl_connect('motion_notify_event', self.onMotion_ToolTip)
        ...
        if self.datatype in ('symetricscalar','scalar'):
            ...
            self.comboscale = wx.ComboBox(self.panel, -1,
                                        self.scaletype,
                                        choices=['Linear', 'Log'],
                                        size=(-1, 40))
...
#             vmax = self.maxvals*(self.ImaxDisplayed/100.)
            ...
            vmax = self.maxvals*(self.ImaxDisplayed/100.)*(self.IminDisplayed/100.)
            vmin = -vmax
            ...
            self.deltavals = (self.maxvals-self.minvals)/100.
        ...
        if self.scaletype == 'Linear':
            ...
            self.cNorm = mpl.colors.LogNorm(vmin=vmin,
                                    vmax=vmax)
        else:
            self.cNorm = None
...
#             print 'self.dataarray_info min ', self.minvals
        ...
        if self.datatype in ('scalar','symetricscalar'):
            ...
            if self.datatype == 'symetricscalar':
                ...
                self.deltavals = (self.maxvals - self.minvals) / 200.
...
#                          extent=self.extent,
    ...
    #                 return index  # self.center[1]-self.boxsize[1]+index
            ...
            def fromindex_to_pixelpos_x_mosaic(index, pos):
                    return np.fix((step_x * index) * 1000.) / 1000.  # self.center[0]-self.boxsize[0]+index
            def fromindex_to_pixelpos_y_mosaic(index, pos):
                    return np.fix((step_y * index) * 1000.) / 1000.  # self.center[1]-self.boxsize[1]+index
    ...
    #         self.axes.locator_params('y', tight=True, nbins=round(nb_of_microns_y) + 1)
            ...
            tickx_sampling_length = max(np.fabs(nb_of_microns_x / 5. / step_x), np.fabs(tickx_micron_sampling))
            ticky_micron_sampling = 1. / step_y
            ticky_sampling_length = max(np.fabs(nb_of_microns_y / 5. / step_y), np.fabs(ticky_micron_sampling))
...
#                     print "self.dataarray_info is None"
                    if self.datatype != 'RGBvector':
                        ...
                        if self.maptype == 'orientation':
                            angle = np.arccos(zvalue)*180./np.pi
                            addedvalue = 'angle(deg)=%.3f'%angle
                        else:
                            addedvalue =''
                        sentence0 = 'x=%1.4f, y=%1.4f, value=%s, %s ImageIndex: %d' % \
                                        (x, y, zvalue, addedvalue, Imageindex)
                    else:
                        zvalue = str(z)
                        sentence0 = 'x=%1.4f, y=%1.4f, value=%s, ImageIndex: %d' % \
                                        (x, y, zvalue, Imageindex)
                    sentence_corner = ''
                    sentence_center = ''
...
class ImshowFrame(wx.Frame):
    ...
    def __init__(self, parent, _id, title, dataarray, absolutecornerindices=None,
                 Size=(4, 3), Imageindices=np.arange(2),
                        nb_row=10,
                        nb_lines=10,
...
#         if self.datatype == 'scalar':
        ...
        if WXPYTHON4:            
            ...
            self.slider_min.SetTickFreq(50, 1)
        self.Bind(wx.EVT_COMMAND_SCROLL_THUMBTRACK, self.OnSliderMin,
                                                self.slider_min)
        ...
        if WXPYTHON4:            
            ...
            self.slider_max.SetTickFreq(50, 1)
        self.Bind(wx.EVT_COMMAND_SCROLL_THUMBTRACK, self.OnSliderMax,
                                                self.slider_max)
        ...
        if self.datatype in ('Vector',):
            ...
            self.slidertxt_arrowwidth = wx.StaticText(self.panel, -1,
                "Arrow width :")
            self.slider_arrowwidth = wx.Slider(self.panel, -1, size=(200, 50),
            ...
            self.slidertxt_arrowscale = wx.StaticText(self.panel, -1,
                "Arrow scale :")
            self.slider_arrowscale = wx.Slider(self.panel, -1, size=(200, 50),
                value=self.arrowWidthDisplayed,
                minValue=1,
                maxValue=100,
                style=wx.SL_AUTOTICKS | wx.SL_LABELS)
...
#         self.vbox.Add(self.btnflipud, 0, wx.EXPAND)
    ...
    def createMenu(self):
        ...
        roi_menu = wx.Menu()
        SaveRoi = roi_menu.Append(1111, "&Capture && Save ROI","Capture current rectangular image indices ROI and save")
        self.Bind(wx.EVT_MENU, self.SaveRectROI, SaveRoi)
        
        SaveAsRoi = roi_menu.Append(1112, "Capture && &Save ROI As","Capture current rectangular image indices ROI and save with user defined name")
...
#             MenuBar.EnableTop(1, False)
    ...
    def OnSave(self, event):
        ...
        dlg = wx.TextEntryDialog(self, 'Enter filename for image',
    ...
    def SaveData(self,event):
        wx.MessageBox('To be implemented, but data are automatically saved in the same folder than the images one','INFO')
    ...
    def OnSliderArrowSize(self,evt):
    ...
    def OnSliderArrowScale(self,evt):
...
#         print "OnChangeLUT"
    ...
    def OnChangeCenter(self,evt,newCenter=None):
        ...
        if self.datatype == 'Vector':
            ...
            radialdistance_2D = np.sqrt((dataX_2D-pixelCenter[0])**2+(dataY_2D-pixelCenter[1])**2)
...
#         self.canvas.draw()
    ...
    def OnMaskWeakPeaks(self):
        ...
        (peak_X, peak_Y,peak_I,
         ...
         peak_inclination, 
         Xdev, Ydev, peak_bkg,
         maskedrows) = self.ApplyMaskOnPeaksProperties(maskthreshold).T
...
#         print "where Masked", np.where(BoolToMask==True)
        ...
        fittedPeaksData = np.ma.array([peak_X, peak_Y,
                        peak_I, peak_fwaxmaj, peak_fwaxmin,
                        peak_inclination, Xdev, Ydev, peak_bkg,FilterX]).T
...
#             print "X",X
            self.myplot = self.axes.quiver(X,Y,V,U,M,
                                           ...
                                           pivot='mid',edgecolors=('k'))
            self.axes.plot(X, Y, 'k.', markersize=2)
            if self.memorizedxlimits is None:
                self.axes.axis([-1, n1, -1, n0])
        else: # 2D imshow scalar plot
                      
            self.myplot = self.axes.imshow(self.data, cmap=self.LastLUT,
                                       interpolation='nearest', origin=self.originYaxis)
            self.colorbar=self.fig.colorbar(self.myplot)
...
#            print("out axes", event.xdata, event.ydata)
    ...
    def OnRightButtonMousePressed(self,evt):
    ...
    def GetXYZmotorsfromCurrentImage(self):
        ...
        imagefilename=RMCCD.setfilename(imagefilenameexample, self.currentpointedImageIndex, 4, CCDLabel)
...
#         print "xyzech",xyzech
    ...
    def onKeyPressed(self, event):
        ...
        if key == 'escape':
            ...
            print('SPEC COMMAND:\nmv %s %.6f %s %.6f' % \
                          (motor1name, posmotor1,
                     motor2name, posmotor2))
...
#                     wx.MessageBox(sentence, 'INFO')
            ...
            msgdialog = PMSG.MessageCommand(self, -1, 'motors command',
                                       sentence=sentence,
                                       speccommand=command,
                                       specconnection=None)
            msgdialog.ShowModal()
...
#         print "self.tabindices", self.tabindices
            if col >= 0 and col < numcols and row >= 0 and row < numrows:
                ...
                if self.datatype in ('RadialPosition','Vector'):
                    ...
                    ret = wx.MessageBox('Set new center of Y data with origin = Y value of image %d\n%.2f'% (absoluteImageIndex,neworigin),
...
#         print "indi,indj",indi,indj
        ...
        if indi<0: indi=0
        if indi>=nlines: indi=nlines-1
        
        if indj<0: indj=0
        if indj>=ncols: indj=ncols-1
...
#                 self.currentpointedImageIndex=self.getImageIndexfromxy(x,y)
                ...
                if self.datatype == 'Vector':
                    ...
                    return 'x=%1.4f, y=%1.4f, Dx=%.2f Dy=%.2f,\nDnorm %.2f, ImageIndex: %d' % (x, y, Dx,Dy,Dnorm,self.currentpointedImageIndex)
                elif self.datatype == 'ReciprocalMap':
                    z = self.data[row, col]
                    qx,qz,qn,E = self.dict_param['QxyznE'][row,col]
                    sentence = 'x=%1.4f, y=%1.4f, I=%5.1f, ImageIndex: %d' % (x, y, z, self.currentpointedImageIndex)
                    sentence+='\n qx=%1.4f, qz=%1.4f, qn=%.5f, Energy: %.4f' % (qx, qz, qn, E)
                    return sentence
                
                else:
                    z = self.data[row, col]
                    return 'x=%1.4f, y=%1.4f, I=%5.5f, ImageIndex: %d' % (x, y, z, self.currentpointedImageIndex)
...
#         print "indlb,indrb,indlt,indrt",indlb,indrb,indlt,indrt
        ...
        ROI_extent_values= (centerimageindex,
                                         nbimagesxwidth,
                                         nbimagesywidth)
...
class DrawFrame(wx.Frame):
    ...
    A frame
    """
    def __init__(self, parent, imagename='defaultName',
                    ImageTIFF=None,
                    nb_row=10, nb_lines=10,
                    boxsize_row=10, boxsize_line=10,
        ...
        if meanvalues == 1:
            ...
            plapla = ImshowFrame(self, -1, 'MOSAIC image Plot', meanvaluestab,
                                cmap=GT.ORRD,
                                interpolation='nearest',
                                origin='upper',
            ...
            # numrows, numcols = self.data.shape
            ploplo = ImshowFrame(self, -1, 'MOSAIC image Plot', np.log(dat),
                                cmap=GT.ORRD,
                                interpolation='nearest',
                                origin='upper',
                                Imageindices=self.tabindices,
...
def myformattime():
    ...
    return 'date_%04d%02d%02d_%02d%02d%02d' % (tt.tm_year, tt.tm_mon, tt.tm_mday,
                                     tt.tm_hour, tt.tm_min, tt.tm_sec)


def buildMosaic3(dict_param, outputfolder,
                 ccdlabel='MARCCD165',
                 plot=1,
                 parent=None):
    """
...
#        print "boxpixelsize", boxpixelsize
        ...
        for map_imageindex, absolute_imageindex in enumerate(selected1Darray_absoluteimageindex):
            ...
            filename = RMCCD.setfilename(filename_representative, imageindex,
                                         CCDLabel=ccdlabel,nbdigits=nbdigits)
...
#                 datacrop = dataimage[imin:imax, jmin:jmax]
                ...
                if not filename.endswith('tif.gz'):
                    ...
                    datacrop = RMCCD.readrectangle_in_image(filename, xpic, ypic, halfboxsizes[0], halfboxsizes[1], 
                                        dirname=None, CCDLabel=ccdlabel)
                else:
                ...
                if dict_param['NormalizeWithMonitor']:
                    ...
                    if ccdlabel in ('sCMOS','sCMOS_fliplr'):
                        ...
                        monitor_val=dictMonitor['mon']-monitoroffset*dictMonitor['exposure']/1000.
        ...
        for counter in selectedcounters:
            ...
            if counter in ('mean', 'max', 'ptp'):
                if counter == 'mean':
                    ...
                    dat = np.amax(np.amax(mosaic, axis=2), axis=2) - \
                                np.amin(np.amin(mosaic, axis=2), axis=2)
...
#                 print "rawdat.shape", rawdat.shape
                ...
                if dict_param['NormalizeWithMonitor']:
                    ...
                    monitor = np.where(monitor<=0.,1.,monitor)
                ...
                if plot:
                    ...
                    plapla = ImshowFrame(parent, -1, 'image Plot %s' % counter,
                                         dat,
                                        cmap=GT.ORRD,
                                        interpolation='nearest',
                                        origin='upper',
...
#                print "lenXY", len(Intens)
                ...
                if plot:
                    ...
                    plotI = PLOT1D.Plot1DFrame(parent, -1, counter + ' Intensity', title + ' Intensity',
                                               XYdat, logscale=0)
                    plotI.Show(True)
...
#                 title = '%s indexrange [%06d-%06d]' % (counter, startind, endind)
    ...
    #            print "shape dat", shape(dat)
                ...
                dat = dat.reshape((nb_lines * (2 * boxsize_line + 1),
                                   nb_col * (2 * boxsize_col + 1)))
    ...
    #            print "shape dat", shape(dat)
                ...
                if plot:
                    ploplo = ImshowFrame(parent, -1, 'MOSAIC image Plot',
                                         dat,
        ...
        #            ploplo.dirname = self.dirname
            ...
            elif counter in ('Position XY', 'Position MAX' , 'Position Centroid','Displacement','Amplitude','Shape'):
                print("\n\n\n ***************************  Position XY****************\n\n\n")
...
#                print 'label', label
                ...
                datminimums = np.repeat(datminimum.reshape((n0, n1)), n2 * n3, axis=1).reshape((n0, n1, n2, n3))
...
#                                                                              arange(n0 * n1)))
                ...
                datcenterofmass2 = np.array(scind.measurements.center_of_mass(mosaic - datminimums,
                                                                              label,
                                                                              np.arange(n0 * n1)))
                # from relative position to absolute initial CCD position
                centerofmass = datcenterofmass2[:, 2:] + np.array([jmin, imin])

                datmaximumpos = scind.measurements.maximum_position(mosaic, label, np.arange(n0 * n1))
...
#                 elif counter == 'Position XY':
                    XY,  FilteredfittedPeaksData = FitPeakOnMap(mosaic,(xpic,ypic),
                        ...
                        reject_weakPeaks = False, FitPixelDev = None,
...
#                print "lenXY", len(XY)
                ...
                if plot and counter == 'Position XY':
                    ...
                    plotX = PLOT1D.Plot1DFrame(parent, -1, counter + 'Xpos',
                                               title + 'Xpos',
                                               XYdat_x, logscale=0)
                    plotX.Show(True)
                    ...
                    plotY = PLOT1D.Plot1DFrame(parent, -1, counter + 'Ypos',
                                               title + 'Ypos',
                                               XYdat_y, logscale=0)
                    plotY.Show(True)
                ...
                if plot and counter == 'Position XY':
                    ...
                    plot2DX = ImshowFrame(parent, -1, 'image 2D Plot X %s' % counter, dataX_2D,
                                        cmap=GT.ORRD,
                                        interpolation='nearest',
                                        origin='upper',
                                        Imageindices=tabindices,
        ...
        #            plapla.dirname = self.dirname
                ...
                # plot 2D Y position
                if plot and counter == 'Position XY':
                    plot2DY = ImshowFrame(parent, -1, 'image 2D Plot Y %s' % counter, dataY_2D,
                                        cmap=GT.ORRD,
                                        interpolation='nearest',
                                        origin='upper',
                                        Imageindices=tabindices,
        ...
        #            plapla.dirname = self.dirname
                    ...
                    plotAmplitude = PLOT1D.Plot1DFrame(parent, -1, counter + 'Amplitude',
                                               title + 'Amplitude',
                                               dataamplitude, logscale=0)
...
def FitPeakOnMap(mosaic,ROIcenter, reject_negative_baseline=True,
                        reject_large_PixelDeviation =True,
                        reject_weakPeaks = True, FitPixelDev = None,
                        MinimumPeakAmplitude = 25,
                        modelFunction='gaussian',
    ...
    FittingParametersDict['start_sigma1']=2.
    FittingParametersDict['start_sigma2']=2.
...
#                     reject_negative_baseline = False
    ...
    if reject_large_PixelDeviation:
        ...
        if len(to_reject3)>0: print('to_reject3  too far from initial guess minimum pixel distance = %f'%FitPixelDev, to_reject3)
    else:
        to_reject3=[]
        
    ToR = set(to_reject)| set(to_reject0) | set(to_reject1) | set(to_reject2) | set(to_reject3)  # to reject

    print("After fitting, %d/%d peaks have been rejected" % \
                                (len(ToR), len(CentralPeaks)))
...
#     print "where Masked", np.where(BoolToMask==True)
    ...
    # all peaks list building
    fittedPeaksData = np.array([peak_X, peak_Y,
                    peak_I, peak_fwaxmaj, peak_fwaxmin,
                    peak_inclination, Xdev, Ydev, peak_bkg,maskedrows]).T
...
def CollectData(param, outputfolder, ccdlabel='MARCCD165'):
    ...
    nb_lines, boxsize_row, boxsize_line = list(map(int,
                                                [nb_lines, boxsize_row, boxsize_line]))
...
def CollectData_oneImage(param, outputfolder, ccdlabel='MARCCD165'):
    ...
    """
    (dirname, filename, imageindex,
            peaklist, boxsize_row, boxsize_line) = param
...
#     print 'peaklist', peaklist
    ...
    CountersData['Monitor'] = 1.
    CountersData['ExposureTime'] = 1000.  # milliseconds
...
def usage():
    print("\n\n**********************\nUsage: python mosaic.py /home/mydata/myimage0125.mccd startind endind nb_lines xpic ypic boxsize_row boxsize_line mosaicflag\n mosaicflag=mosaic for mosaic, =mean for only mean value mosaic plotted\n**********************\n\n")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # print len(sys.argv)
        if len(sys.argv) == 9:
            dir_file, startind, endind, nb_lines, xpic, ypic, boxsize_row, boxsize_line = sys.argv[1:]
            ...
            prefix = allprefix[:-4]
            startind, endind = int(startind), int(endind)
            nb_lines, xpic, ypic, boxsize_row, boxsize_line = list(map(int, [nb_lines, xpic, ypic, boxsize_row, boxsize_line]))

        elif len(sys.argv) == 10:
            dir_file, startind, endind, nb_lines, xpic, ypic, boxsize_row, boxsize_line, flag = sys.argv[1:]
            if flag == 'mosaic':
                meanvalues = 0
            elif flag == 'mean':
                meanvalues = 1
    ...
    try:
        ...
        for k in tabindices:
            ...
            if k % 10 == 1: print(filename, "up to", prefix + stringint(endind, 4) + '.' + fileextension)
            kx, ky = dic_carto[k][1:]
            ...
            mosaic.paste(region, (ky * 2 * boxsize_row, kx * 2 * boxsize_line, (ky + 1) * 2 * boxsize_row, (kx + 1) * 2 * boxsize_line))
        ...
        F = DrawFrame(None, imagename=os.path.join(didi, imagefilename),
                ImageTIFF=imageout,
                nb_row=nb_row, nb_lines=nb_lines,
                boxsize_row=boxsize_row, boxsize_line=boxsize_line,
                tabindices=tabindices, meanvalues=meanvalues)
        F.dirname = didi
