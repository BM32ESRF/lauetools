...
class LinkEditor(wx.Frame):
    def __init__(
        self, parent, _id, title, previouslist, millerlist, intensitylist=None
    ):
        ...
        vbox3.AddMany(
            [
                (wx.StaticText(pnl1, -1, "Experimental"), 0, wx.ALIGN_CENTER),
    ...
    def OnAdd(self, event):
        ...
        try:
            self.lc.SetStringItem(
                num_items, 2, str(self.ReadMiller(int(self.tc2.GetValue())))
            )
        except IndexError:
            wx.MessageBox(
                "Theoretical spot of index %d does not exist ! \n Please remove it! "
                % int(self.tc2.GetValue()),
                "INFO",
            )

        if self.intensitylist is not None:
            self.lc.SetStringItem(
                num_items, 3, str(self.intensitylist[int(self.tc1.GetValue())])
            )
        else:
            self.lc.SetStringItem(num_items, 3, "unknown")
