...
def computefilteredimage(dataimage, bkg_image, CCDlabel, kernelsize=5, formulaexpression="A-B",
    ...
    if CCDlabel in ("sCMOS", "sCMOS_16M","sCMOS_9M","sCMOS_fliplr", "EIGER_4M", "EIGER_4MCdTe",'MaxiPIXCdTe','Alban', 'psl_IN_tif', 'Alexiane','EIGER_1M'):
        usemask = False
    ...
    if CCDlabel in ("MaxiPIXCdTe","EIGER_4MCdTe"): # mask dead pixels 
        dataimage = np.ma.masked_where(dataimage<=0, dataimage)
        bkg_image = np.ma.masked_where(bkg_image<=0, bkg_image)
