...
class MainFrame(wx.Frame):
    ...
    def createMenuBar(self):
        ...
        filemenu = wx.Menu()
        menuSpecFile = filemenu.Append(-1, "Open spec file",
                                         "Open a spec file")
        menuSetPreference = filemenu.Append(-1, "Folder Preferences",
                                         "Set folder Preferences")
        savemeshdata = filemenu.Append(-1, "Save Data",
                                         "Save current 2D data")
...
#         print dir(self.stbar)
        ...
        self.plot = ImshowPanel(self.panel, -1, 'test_plot', z_values,
...
#         vbox.Add(self.stopbtn, 0, wx.BOTTOM)
    ...
    def OnSaveData(self, evt):
        ...
        file = wx.FileDialog(self, "Save 2D Array Data in File", defaultDir=defaultdir,
...
#         self.flat_motor2 = posmotor2
            ...
            self.writefile_3columns(outputfile,
    ...
    def writefile_3columns(self, output_filename, data):
        ...
        outputfile.write('\n'.join(['%.06f   %.06f   %06f' % tuple(zip(data[0], data[1], data[2])[i]) \
    ...
    def OnOpenSpecFile(self, evt):
        ...
        folder = wx.FileDialog(self, "Select spec file",
...
#         print "dtt", dir(self.treespecfiles.tree)
    ...
    def ReadScan_SpecFile(self, scan_index):
        ...
        if self.normalizeintensity:
            ...
            counterintensity1D = datay / (data_I0 / (exposureTime / 1.) - self.MonitorOffset)
...
#         print "posmotorsinfo", posmotorsinfo
        ...
        scancommandextremmotorspositions = [self.minmotor1,self.maxmotor1,self.minmotor2,self.maxmotor2]
    ...
    def update_fig(self, data_z_values,
                   posmotorsinfo, motor1, motor2,
                   Apptitle,
                   data_img,
                   detectorname,
                   scancommandextremmotorspositions):
...
#         self.plot.fig.clear()
        ...
        (self.plot.minmotor1,
         self.plot.maxmotor1,
         self.plot.minmotor2,
         self.plot.maxmotor2)=scancommandextremmotorspositions
        ...
        if self.plot.colorbar is not None:
            ...
            (self.plot.myplot,
             self.plot.colorbar,
             self.plot.data) = makefig_update(self.plot.fig,
                                              self.plot.myplot,
                                              self.plot.colorbar,
                                              data_z_values)
        else:
...
#         if self.datatype == 'scalar':
        self.slidertxt_min = wx.StaticText(self, -1,
            "Min :")
        self.slider_min = wx.Slider(self, -1, size=(200, 50),
        ...
        self.slidertxt_max = wx.StaticText(self, -1,
            "Max :")
        self.slider_max = wx.Slider(self, -1, size=(200, 50),
        ...
        self.combocounters = wx.ComboBox(self, -1,
...
#         print "self.step_x", self.step_x
        return np.fix((index * self.step_x / step_factor + self.posmotor1[0]) * 100000.) / 100000.
    ...
    def fromindex_to_pixelpos_x_relative_corner(self, index, pos):
        ...
        return np.fix((index * self.step_x) * 10000.) / 10000.
...
#         return np.fix((index * self.step_x) * 100.) / 100.
    ...
    def fromindex_to_pixelpos_y_absolute(self, index, pos):
        ...
        print("step_factor", step_factor)
        print("self.step_x", self.step_x)
        return np.fix((index * self.step_y / step_factor + self.posmotor2[0]) * 100000.) / 100000.
    ...
    def fromindex_to_pixelpos_y_relative_corner(self, index, pos):
        ...
        return np.fix((index * self.step_y) * 10000.) / 10000.
...
# #             self.forceAspect(ratio)
    ...
    def set_motorspositions_parameters(self):
        ...
        print("second motor",self.posarray_twomotors[-1, 0], self.posarray_twomotors[-1, -1])
        
        rangeX = np.fix((self.posarray_twomotors[0, -1]-self.posarray_twomotors[0, 0])[0]*100000)/100000
        rangeY = np.fix((self.posarray_twomotors[-1, -1]-self.posarray_twomotors[0, -1])[1]*100000)/100000
...
#                 print "posmotor1[col],posmotor2[row]", posmotor1[col], posmotor2[row]
                ...
                sentence0 = "j=%d, i=%d, ABSOLUTE=[%s=%.5f,%s=%.5f], z_intensity = %s, ImageIndex: %d" % \
                        (col, row, 
                         self.motor1name, posmotor1[col], self.motor2name, posmotor2[row],
                         str(z),Imageindex)
