...
def show_solution_existence():
    ...
    bestangles = [37, 7, 89]
    IMM.Plot_compare_2thetachi(bestangles, Twicetheta, Chi, verbose=0, key_material='Si', emax=25,
                           EULER=EULER)
...
#    detectorparameters = [68.2, 920, 880, -0.1, 0.8]
    ...
    TBO, dataselected, gno_dataselected = IMM.give_best_orientations_new('testSirandom.cor',
                                                                1000000,
                                                                database,
                                                                dirname=None,
                                                                plotHough=0)
...
#                            key_material='Si',EULER=1)
    ...
    for k in list(range(len(bestEULER))):
        p.close()
        IMM.Plot_compare_2thetachi(bestEULER[k], dataselected[0], dataselected[1],
...
#    detectorparameters = [70, 1024, 1024, 0, 0]
    ...
    correlintensity, argcorrel = IMM.plotHoughCorrel(database, 'testSirandom.cor', 100000,
                                                 dirname='.', returnCorrelArray=1)
#    print "min", np.amin(correlintensity)
#    print "max", np.amax(correlintensity)
#    print "mean", np.mean(correlintensity)
...
#    print bestEULER
    ...
    sortedindices, matchingrates = matchingrate.getStatsOnMatching(bestEULER, twicetheta_data, chi_data, 'Si', verbose=0)
...
#        p.close()
    import pylab as p
    p.close()
    IMM.Plot_compare_2thetachi(bestEULER[0], dataselected[0], dataselected[1],
                           verbose=1, key_material='Si', emax=25,
                           EULER=1)
...
#    print bestEULER
    ...
    # sort solution by matching rate
    sortedindices, matchingrates = matchingrate.getStatsOnMatching(bestEULER, twicetheta_data, chi_data, ELEMENT, verbose=0)
...
#    print "indexed_spots_dict", indexed_spots_dict
    ...
    TBO, dataselected, gno_dataselected = IMM.give_best_orientations_new(file_to_index,
                                                                1000000,
                                                                database,
                                                                maxindex=maxindex,
                                                                dirname=None,
...
#    print bestEULER
    ...
    if dict_grain_matrix is not None:
        ...
        IMM.Plot_compare_2thetachi(bestEULER_0[0], dataselected[0], dataselected[1],
                           verbose=1, key_material=ELEMENT, emax=25,
                           EULER=1)

        IMM.Plot_compare_gnomondata(bestEULER_0[0], dataselected[0], dataselected[1],
                           verbose=1, key_material=ELEMENT, emax=25,
                           EULER=1)
        return
...
def test_ImageMatching_index(database=None, nbgrains=3, readfile=None):
    ...
    No intensity used
    """
    from indexingSpotsSet import (initIndexationDict,
...
#        nb_of_peaks = 100
    ...
    bestEULER = IMM.bestorient_from_2thetachi((Twicetheta, Chi), database, dictparameters=dictimm)
    ...
    if dict_grain_matrix is not None:
        ...
        print("plot the best orientation matrix candidate")
        IMM.Plot_compare_2thetachi(bestEULER[0], twicetheta_data, chi_data,
                           verbose=1, key_material=key_material, emax=emax,
                           EULER=1)

        IMM.Plot_compare_gnomondata(bestEULER[0], twicetheta_data, chi_data,
                           verbose=1, key_material=key_material, emax=emax,
                           EULER=1)
        return
...
def test_old_imagematching(database):
    ...
    bestEULER, bestmatchingrates = ISS.filterEulersList(bestEULER, TwiceTheta_Chi_Int, key_material, emax,
                                                    rawAngularmatchingtolerance=firstmatchingtolerance,
                                                    MatchingRate_Threshold=MatchingRate_Threshold,
                                                    verbose=1)
