...
def test_1():
    ...
    IIw0 = 10.
    yoyo = LGeo.IW_from_wireabscissa(abscissa, IIw0, anglesample=40.)
    tata = LGeo.Wireabscissa_from_IW(yoyo[0], yoyo[1], IIw0, anglesample=40.)
...
def test_2():
    ...
    uflab_2, IMlab_2 = LGeo.calc_uflab(xcam, ycam, calib, returnAngles='uflab', verbose=0)
    print("****************")
    print("start", uflab)
    print("final", uflab_2)
    print("This two arrays must be equal!")
...
def test_4(IIprime, height_wire, errorOM1, errorOM2):
    ...
    print("2 IWs", IWs)
    print("------------")
    ysource, zsource = LGeo.find_yzsource_from_2xycam_2yzwire(OMs, IWs, calib, anglesample=anglesample)
    print("ysource,zsource", ysource, zsource)
...
def test_5(IIprime, height_wire, arrayindex, errorWabscissa1, errorWabscissa2):
    ...
    _twtheta, _chi = LGeo.calc_uflab(OM[0], OM[1], calib, returnAngles=1, verbose=0, pixelsize=165. / 2048)
    ...
    print("------------")
    ysource, zsource = LGeo.find_yzsource_from_2xycam_2yzwire_version2(OMs, IWs, calib, anglesample=anglesample)
...
def test_offset_xraysource():
    ...
    X, Y, th0, E = LGeo.calc_xycam(uflab, calib, energy=1, offset=[0, 0, 0.])
    ...
    print("\n With offset\n")
    X, Y, th0, E = LGeo.calc_xycam(uflab, calib, energy=1, offset=[0., .01, 0.])
    print("Xcam (mm)", X * 165 / 2048.)
    print("Ycam (mm)", Y * 165 / 2048.)
    print("Xcam (pixel)", X)
    print("Ycam (pixel)", Y)
...
def test_sourcetriangulation():
    ...
    X0, Y0, th0 = LGeo.calc_xycam(uflab, calib, energy=0, offset=posI, verbose=0, returnIpM=False)
    print(X0, Y0)
    print("IM0", IM0)
    depth_wire = 0.01
    IW0y, IW0z = LGeo.IW_from_IM_onesource(posI[1:], IM0, depth_wire, anglesample=40.)
    ...
    IM1 = IpM + posI
    X1, Y1, th1 = LGeo.calc_xycam(uflab, calib, energy=0, offset=posI, verbose=0, returnIpM=False)
    print("X1", X1)
    print("Y1", Y1)
    print("IM1", IM1)

    H_wire = 0.01
    IW1y, IW1z = LGeo.IW_from_IM_onesource(posI[1:], IM1, H_wire, anglesample=40.)

    print("IW1y,IW1z", IW1y, IW1z)
    Wireabscisa_1 =LGeo.Wireabscissa_from_IW(IW1y, IW1z, H_wire, anglesample=40.)
    ...
    ysource, zsource = LGeo.find_yzsource_from_2xycam_2yzwire(OMs, IWs, calib, anglesample=40.)
...
def test_sourcefinding():
    ...
    sourcepos = LGeo.find_multiplesourcesyz_from_multiplexycam_multipleyzwire(OMs, Wire_abscissae, calib, anglesample=40., wire_height=H_wire)
...
def test_correction_1():
    ...
    print(twicetheta)
    IOLT.writefile_cor('polyZrO2_test', twicetheta, chi, data_x, data_y, dataintensity, param=param, initialfilename=peaksfilename)
...
def test_correction_2():
    ...
    print(twicetheta)
    IOLT.writefile_cor('Ge_test', twicetheta, chi, data_x, data_y, dataintensity, param=param, initialfilename=peaksfilename)
...
def test_correction_3():
    ...
    print(twicetheta)
    IOLT.writefile_cor('Ge_run41_1_0003', twicetheta, chi, data_x, data_y, dataintensity, param=param, initialfilename=peaksfilename)
...
def test_Gec(anglesample=40.,
    ...
    Calculation: 
    """
    res_falling = LGeo.find_multiplesourcesyz_from_multiplexycam_multipleyzwire(OMs, Wire_abscissae_falling, calib, anglesample, wire_height, 0)
    res_rising = LGeo.find_multiplesourcesyz_from_multiplexycam_multipleyzwire(OMs, Wire_abscissae_rising, calib, anglesample, wire_height, 0)
...
if __name__ == "__main__":
    ...
    IWy1, IWz1 = LGeo.IW_from_IM_onesource(IIprime[1:], IMlab1, height_wire, anglesample=anglesample, anglewire=anglesample)
    ...
    IWy2, IWz2 = LGeo.IW_from_IM_onesource(IIprime[1:], IMlab2, height_wire, anglesample=anglesample, anglewire=anglesample)
    ...
    def series():
        ...
        for index in list(range(620, 1276)):
            ...
            twicetheta, chi, dataintensity, data_x, data_y = LGeo.Compute_data2thetachi(filename, (col_X, col_Y, col_I), nblines_headertoskip)
