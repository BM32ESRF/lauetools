...
class PlotRangePanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.shiftChiOrigin = wx.CheckBox(self, -1,
                                          'Shift Chi origin of Exp. Data',
                                          (2, pos5b + 60))
        ...
        self.shiftChiOrigin.SetToolTipString('Check to shift the chi angles of experimental spots by the central chi value')
...
class CrystalParamPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.eminC = wx.SpinCtrl(self, -1, '5', (150 + deltaposx, pos6 - 5),
                                (60, -1), min=5, max=149)
        self.emaxC = wx.SpinCtrl(self, -1, '22', (180 + deltaposx + 50, pos6 - 5),
                                (60, -1), min=6, max=150)
        ...
        self.comboMatrix = wx.ComboBox(self, 2525,
                                      'Identity',
                                     (150 + deltaposx, pos7b),
        ...
        self.comboExtinctions = wx.ComboBox(self, -1,
                                      'Diamond',
                                     (150 + deltaposx, pos7c),
...
#         self.comboTransforms.Bind(wx.EVT_COMBOBOX, self.mainframe.OnChangeTransforms)
        ...
        b2 = wx.Button(self, 1011, 'Store UB', (deltaposx + 150, pos7d + vertical_shift), (100, 40))
        btn_sortUBsname = wx.Button(self, 1011, 'sort UBs name', (deltaposx + 300, pos7d + vertical_shift), (100, 40))
...
#         self.Bind(wx.EVT_BUTTON, self.mainframe.OnInputParam, id=101)
        ...
        self.detectordiameter_txtctrl = wx.TextCtrl(self, -1, str(self.mainframe.detectordiameter))
...
class MoveCCDandXtal(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.rotatebtn = wx.Button(self, -1, self.EnableRotationLabel, (posx, 180), (100, 100))
        ...
        self.listofparamfitctrl = [self.cb_dd,
                                   self.cb_Xcen,
                                   self.cb_Ycen,
                                   self.cb_angle1,
...
class StrainXtal(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        for k, key_param in enumerate(self.lattice_parameters_key):
            ...
            fitchckbox.Disable()
            currentctrl = wx.TextCtrl(self, -1,
...
#             print getattr(self, 'minusbtn_%s' % key_param)
            ...
            getattr(self, 'plusbtn_%s' % key_param).Bind(wx.EVT_BUTTON,
                                        lambda event: self.ModifyLatticeParamsStep(event, '+'))
            getattr(self, 'minusbtn_%s' % key_param).Bind(wx.EVT_BUTTON,
                                        lambda event: self.ModifyLatticeParamsStep(event, '-'))
...
#         print "my parent is ", parent
        ...
        matrixText = wx.TextCtrl(panel, -1, strexpression, size=(490, 100), style=wx.TE_MULTILINE | wx.TE_READONLY)
#         matrixText.SetInsertionPoint(0)
        ...
        sizer6.AddMany([storeLabel, self.storeText, btnstore,
                       saveLabel, self.saveText, btnsave])
    ...
    def onSave(self, evt):
        ...
        text = "[[%.17f,%.17f,%.17f],\n[%.17f,%.17f,%.17f],\n[%.17f,%.17f,%.17f]]" % tuple(np.ravel(UBmatrix).tolist())
        _file.write(text)
        _file.close()
...
#        self.plotPanel = wxmpl.PlotPanel(self, -1, size=Size, autoscaleUnzoom=False)
        ...
        self.Bind(wx.EVT_BUTTON, self._replot, id=52)  # replot simul button (one button in two panels)
        ...
        self.btn_label_theospot = wx.ToggleButton(self.panel, 104, 'Label Exp. spot',
                                            size=(150, 40))
        self.btn_label_expspot = wx.ToggleButton(self.panel, 106, 'Label Simul. spot',
                                                 size=(150, 40))
...
#         self.define_kf_direction()
        ...
        self.moveccdandxtal.SetToolTipString('Change manually and fit (by checking corresponding boxes) the 5 CCD parameters and rotate the crystal around 3 elementary angles')
        ...
        self.btnmanuallinks.SetToolTipString('Build manually a list of associations or links between close simulated and experimental spots')
        self.btnautolinks.SetToolTipString('Build automatically a list of associations or links between close simulated and experimental spots within "Angle Tolerance"')
        ...
        self.btnswitchspace.SetToolTipString('switch between different spots coordinates: 2theta,chi ; Gnomonic projection coordinates ; X,Y pixel position on Camera')
        ...
        self.btn_label_expspot.SetToolTipString('Display on plot data related to selected (by clicking) theoretical (simulated) spot: #index, hkl miller indices, Energy')
        ...
        self.btnsavecalib.SetToolTipString('Save .det file containing current CCD parameters and current crystal orientation')
        self.btnsaveresults.SetToolTipString('Save .fit file containing indexed spots used to refine the CCD detector plane and pixel frame parameters.')
    ...
    def ReadExperimentData(self):
        ...
        if extension in ('dat', 'DAT'):
            ...
            (twicetheta, chi, \
             dataintensity, \
             data_x, data_y) = F2TC.Compute_data2thetachi(self.filename,
...
#                 print k, data_x[k], data_y[k], twicetheta[k], twicetheta[k] / 2., chi[k]
            ...
            (alldata,
            data_theta, chi, data_x, data_y, dataintensity,
            detParam) = IOLT.readfile_cor(self.filename)
    ...
    def OnSaveCalib(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            m11, m12, m13, m21, m22, m23, m31, m32, m33 = np.ravel(self.UBmatrix).round(decimals=7)

            dd, xcen, ycen, xbet, xgam = self.CCDParam

            outputfile = open(self.filenameCalib, 'w')
            text = "%.3f, %.2f, %.2f, %.3f, %.3f, %.5f, %.0f, %.0f\n" % \
                    (round(dd, 3), round(xcen, 2), round(ycen, 2), round(xbet, 3), round(xgam, 3), self.pixelsize, round(self.framedim[0], 0), round(self.framedim[1], 0))
...
#                       'framedim', 'detectordiameter', 'kf_direction']
            vals_list = [round(dd, 3), round(xcen, 2), round(ycen, 2), round(xbet, 3), round(xgam, 3),
...
#             os.remove(self.initialParameter['filename'])
    ...
    def OnStoreMatrix(self, event):
        ...
        tf = TextFrame(self, -1, self.getstringrep(self.crystalparampanel.UBmatrix), self.storedmatrixindex)
        tf.Show(True)
...
#             dlg.Destroy()
    ...
    def OnLinkSpots(self, event):  # manual links
        ...
        dia = SLE.LinkEditor(None, -1, 'Link between spots Editor',
                             self.linkExpMiller, self.Miller_ind, intensitylist=self.Data_I)
...
#
    ...
    def StartFit(self, event):
        ...
        """
        if self.linkedspots == []:
            wx.MessageBox('You need to create first links between experimental and simulated spots with the "link spots" button.', 'INFO')
            event.Skip()
            return
        ...
        pixX, pixY = np.take(self.data_x, exp_indices), np.take(self.data_y, exp_indices)  # pixel coordinates
        ...
        listparam = ['distance(mm)', 'Xcen(pixel)', 'Ycen(pixel)', 'Angle1(deg)', 'Angle2(deg)',  # detector parameter
                'theta1(deg)', 'theta2(deg)', 'theta3(deg)']  # misorientation with respect to initial UBmatrix(/ elementary axis rotation)
        ...
        if nb_pairs < nb_fittingparams :
            wx.MessageBox('You need at least %d spots links to fit these %d parameters.' \
                                            % (nb_fittingparams, nb_fittingparams), 'INFO')
            event.Skip()
            return

        print("Initial error--------------------------------------\n")
        residues, deltamat, newmatrix = FitO.error_function_on_demand_calibration(initial_values,
                                        Data_Q,
                                        allparameters,
                                        ...
                                        pureRotation=pureRotation, verbose=1,
        ...
        # display fit results
        dataresults = allresults.tolist() + [np.mean(self.residues_fit)] + [len(self.residues_fit)]
...
#             print "updating orientation parameters"
        ...
        (twicetheta, chi,
                 dataintensity,
                 data_x, data_y) = F2TC.Compute_data2thetachi(fullpathfilename,
...
#         print 'prefix',prefix
    ...
    def OnWriteResults(self, evt):
        ...
        latticeparam = DictLT.dict_Materials[str(elem)][1][0] * 1.
        ...
        peak_bkg, IntensityMax) = selected_data_peak.T

        Xdev_calibFit, Ydev_calibFit = spotsData[4:6]

        Columns = [indExp, intens, _h, _k, _l, Xtheo, Ytheo,
                    Xexp, Yexp,
                    Xdev_calibFit, Ydev_calibFit,
                    residues_calibFit,
                    twthe, chi,
                    Energy,
...
#         dict_matrices['devstrain'] = self.deviatoricstrain
        ...
        IOLT.writefitfile(outputfilename, datatooutput, len(indExp),
...
#             print "Disable Rotation around axis"
    ...
    def OnSetCCDParams(self, event):
        ...
        try:
            ...
            dlg = wx.MessageDialog(self, 'Detector Parameters in entry field are not float values! ', 'Incorr', wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    def OnInputParam(self, event):
        ...
        DPBoard = DP.DetectorParameters(self, -1,
                                     'Detector parameters Board',
                                     self.initialParameter)
        ...
        print("new param", self.CCDParam + [self.pixelsize, self.framedim[0], self.framedim[1], self.detectordiameter, self.kf_direction])
...
#         self.display_current()
    ...
    def OnChangeElement(self, event):
        ...
        self.sb.SetStatusText("Selected Material: %s" % \
                              str(DictLT.dict_Materials[key_material]))
...
#             self.CCDParam[0] = 20.
    ...
    def OnIncreaseXcen(self, event):
        ...
        self.CCDParam[1] += float(self.moveccdandxtal.stepXcen.GetValue())
        if self.CCDParam[1] > 6000.:
            print("Xcen seems too high...")
            self.CCDParam[1] = 6000.
    ...
    def OnDecreaseYcen(self, event):
        ...
        if self.CCDParam[2] < -3000.:
            print("Ycen seems too low...")
            self.CCDParam[2] = -3000.
    ...
    def OnIncreaseYcen(self, event):
        ...
        self.CCDParam[2] += float(self.moveccdandxtal.stepYcen.GetValue())
        if self.CCDParam[2] > 6000.:
            print("Ycen seems too high...")
            self.CCDParam[2] = 6000.
...
#             self.CCDParam[4] = 60.
    ...
    def OnDecreaseAngle3(self, event):
        ...
        mat = np.array([[math.cos(a3), math.sin(a3), 0],
                        ...
                        [0., 0, 1]])  # XMAS and LaueTools are similar
        self.deltamatrix = mat
...
#         print "kf_direction chosen:", self.kf_direction
    ...
    def onSetOrientMatrix_with_BMatrix(self, evt):
        ...
        self.crystalparampanel.UBmatrix = np.dot(self.crystalparampanel.UBmatrix,
                                                 self.Bmatrix)
    ...
    def simulate_theo(self, removeharmonics=0):
        ...
        if self.manualmatrixinput is None:
            self.crystalparampanel.UBmatrix = np.dot(self.deltamatrix,
                                                self.crystalparampanel.UBmatrix)
            # reset deltamatrix
            self.deltamatrix = np.eye(3)
        ...
        # from combobox of UBmatrix
            ...
            self.crystalparampanel.UBmatrix = \
                    DictLT.dict_Rot[self.crystalparampanel.comboMatrix.GetValue()]
...
#        self.kf_direction = LaueToolsframe.kf_direction
        ...
        Grain = CP.Prepare_Grain(self.key_material,
                                 OrientMatrix=self.crystalparampanel.UBmatrix)
        ...
        if SINGLEGRAIN:  # for single grain simulation
            if self.kf_direction in ('Z>0', 'X>0') and removeharmonics==0:
                ...
                ResSimul = LAUE.SimulateLaue_full_np(Grain, self.emin, self.emax, self.CCDParam[:5],
                                        kf_direction=self.kf_direction,
                ...
                ResSimul = LAUE.SimulateLaue(Grain, self.emin, self.emax, self.CCDParam[:5],
                                        kf_direction=self.kf_direction,
...
#             twins_operators = [rot180]
            ...
            (twicetheta, chi,
             self.Miller_ind,
             posx, posy,
             Energy) = LAUE.SimulateLaue_twins(Grainparent, twins_operators,
                                       self.emin, self.emax, self.CCDParam[:5],
...
#             label, latticeparams, extinc = DictLT.dict_Materials[key_material]
#
#             newbeta = 360. - latticeparams[4]
#
#             newlatticeparams = copy.copy(latticeparams)
#             newlatticeparams[4] = newbeta
#
#             DictLT.dict_Materials[key_material + 'twin'] = [key_material + 'twin',
#                                                         newlatticeparams,
#                                                         extinc]
#
#             Graintwin = CP.Prepare_Grain(key_material + 'twin', OrientMatrix=Grain[2])
#             Grains = [Grain, Graintwin]
#
#             (twicetheta, chi,
#              self.Miller_ind,
#              posx, posy,
#              Energy) = LAUE.SimulateLaue_merge(Grains,
#                                        self.emin, self.emax, self.CCDParam[:5],
#                                        only_2thetachi=False,
#                                          kf_direction=self.kf_direction,
#                                          ResolutionAngstrom=False,
...
#
#         # handy solid angle to simulate Laue spots (25 % safety coef)
#         OpeningAngleCollection = 1.25 * np.arctan(0.5 * self.detectordiameter / self.CCDParam[0]) / DEG
#
#         # simulate q and hkl vectors
# #         print "Grain in MainCalibrationFrame simulate_theo"
# #         print Grain
#         # array(vec) and array(indices)  of spots exiting the crystal in 2pi steradian(Z>0)
#         spots2pi = LAUE.getLaueSpots(DictLT.CST_ENERGYKEV / self.emax,
#                                      DictLT.CST_ENERGYKEV / self.emin,
#                                     [Grain], [[""]],
#                                     fastcompute=0,
#                                     fileOK=0,
#                                     verbose=0,
#                                     kf_direction=self.kf_direction,
#                                     OpeningAngleCollection=OpeningAngleCollection)
#
#         # 2theta, chi of spot which are on camera(with harmonics)
# #         print "detectordiameter in simulate_theo", detectordiameter
# #         print "self.framedim[0]", self.framedim[0]
# #         print "self.pixelsize", pixelsize
#         print "Number of spots: ", len(spots2pi[0][0])
#
...
#                                             dim=self.framedim)
#
#         # TODO: gather theses expressions to use only one TwicethetaChi[0]??
#         twicetheta = [spot.Twicetheta for spot in TwicethetaChi[0]]
#         chi = [spot.Chi for spot in TwicethetaChi[0]]
#         self.Miller_ind = [list(spot.Millers) for spot in TwicethetaChi[0]]
#         Energy = [spot.EwaldRadius * CST_ENERGYKEV for spot in TwicethetaChi[0]]
#         #----------------------------------
#
# #         print "self.initialParameter['CCDParam'] in simulate_theo()", self.initialParameter['CCDParam']
#         print "self.CCDParam[:5]", self.CCDParam[:5]
#
#         print "kf_direction", self.kf_direction
#
...
#                                             signgam=DictLT.SIGN_OF_GAMMA,
#                                             dim=self.framedim,
#                                             kf_direction=self.kf_direction)[:2]
...
#             print 'len(np.array(self.data_theo[2]))', len(np.array(self.data_theo[2]))
            Fsquare = 50. / np.sum(np.array(self.data_theo[2]) ** 2, axis=1)
...
#             print "Fsquare", Fsquare
            ...
            if self.datatype == '2thetachi':
                ...
                self.axes.scatter(self.data_theo[0], self.data_theo[1],
                                  s=sizespot, marker='o',
                ...
                self.axes.scatter(self.sim_gnomonx, self.sim_gnomony, s=sizespot,
                                        marker='o', edgecolor='r', facecolor='None')

            elif self.datatype == 'pixels':
                # dependent of matplotlib and OS see pickyframe...
                # self.axes.scatter(self.data_theo[0], self.data_theo[1],s=sizespot, marker='o',alpha=0, edgecolor='r',c='w')  # don't work with linux and matplotlib 0.99.1
                # self.axes.scatter(self.data_theo[0], self.data_theo[1],s=sizespot, marker='o',alpha=0, edgecolor='r',c='w')
                self.axes.scatter(self.data_theo[3], self.data_theo[4],
                                  s=sizespot, marker='o',
                                edgecolor='r', facecolor='None')
        ...
        if self.datatype == '2thetachi':
            ...
            self.axes.scatter(self.twicetheta, self.chi + originChi,
                              s=self.Data_I / np.amax(self.Data_I) * 100.,
                              c=self.Data_I / 50.,alpha=0.5)
...
#                ylim = (-40, 40)
            ...
            if self.init_plot:
                ...
                max2theta = min(180., mean2theta + amp2theta)
                
                minchi = max(-180, meanchi - ampchi)
                maxchi = min(180., meanchi + ampchi)
            ...
            self.axes.scatter(self.data_gnomonx, self.data_gnomony,
                              s=self.Data_I / np.amax(self.Data_I) * 100., c=self.Data_I / 50.,alpha=0.5)
    ...
    def SelectOnePoint(self, event):
        ...
        self.Data_index_expspot = np.arange(len(self.twicetheta))
        """
        xdata, ydata, annotes = self.twicetheta, self.chi, list(zip(self.Data_index_expspot, self.Data_I))
    ...
    def SelectThreePoints(self, event):
        ...
        if self.nbclick_zone <= 3:
            if self.nbclick_zone == 1: self.threepoints = []
            xtol = .5
            ytol = .5
            """
            self.twicetheta, self.chi, self.Data_I, self.File_NAME = self.data
    ...
    def SelectSixPoints(self, event):
        ...
        if self.nbclick_zone <= 6:
            if self.nbclick_zone == 1: self.sixpoints = []
            xtol = 2.
            ytol = 2.
            """
            self.twicetheta, self.chi, self.Data_I, self.File_NAME = self.data
    ...
    def OnInputMiller(self, evt):
        ...
        if self.nbsuccess == 2:
            ...
            mil2 = self.textentry()
            print(mil2)
            tdist = np.arccos(np.dot(mil1, mil2) / np.sqrt(np.dot(mil1, mil1) * np.dot(mil2, mil2))) * 180. / np.pi
    ...
    def readlogicalbuttons(self):
        ...
        return [self.btn_label_theospot.GetValue(),
                self.btn_label_expspot.GetValue()]
    ...
    def select_2pts(self, evt):  # pick distance
        ...
        if self.nbclick_dist <= 2:
            if self.nbclick_dist == 1: self.twopoints = []
    ...
    def Reckon_2pts(self, evt):  # Recognise distance
        ...
        if twospots:
            ...
            sol = INDEX.twospots_recognition([spot1[0] / 2., spot1[1]],
                                             [spot2[0] / 2., spot2[1]],
                                             ang_tol)
            print("sol = ", sol)
            ...
            if type(sol) == type(np.array([1, 2])):
                ...
                for k in range(len(sol[0])):
                    theodist = np.arccos(np.dot(sol[0][k], sol[1][k]) / np.sqrt(np.dot(sol[0][k], sol[0][k]) * np.dot(sol[1][k], sol[1][k]))) * 180. / np.pi
                    # print sol[0][k]
                    # print sol[1][k]
                ...
                for k in range(len(sol[0])):
                    ...
                    spots2pi = LAUE.getLaueSpots(DictLT.CST_ENERGYKEV / emax, DictLT.CST_ENERGYKEV / emin, [grain], [[""]], fastcompute=1, fileOK=0, verbose=0)
                    # 2theta, chi of spot which are on camera(with harmonics)
                ...
                if self.datatype == 'gnomon':
                    ...
                    self.RecBox = RecognitionResultCheckBox(self, -1,
                                    'Potential solutions', res, self.data_fromGnomon, emax=emax)
                    ...
                    self.RecBox = RecognitionResultCheckBox(self, -1,
                                'Potential solutions', res, self.data, emax=emax)
                    self.RecBox = str(self.crystalparampanel.comboElem.GetValue())
                    self.RecBox.TwicethetaChi_solution = self.TwicethetaChi_solution
                    self.RecBox.mat_solution = self.mat_solution
                print("result", res)
...
#         print "twth2, chi2", twth2, chi2
        ...
        # right mouse button
            ...
            (self.SelectedRotationAxis,
             angle) = self.computeRotation_aroundaxis(axis2theta, axischi, twth1, chi1, twth2, chi2)
...
#         print "self.SelectedRotationAxis, angle", self.SelectedRotationAxis, angle
    ...
    def computeRotation(self, twth1, chi1, twth2, chi2):
        ...
        """
        q1 = np.array([-np.sin(twth1 / 2.*DEG),
                       np.cos(twth1 / 2.*DEG) * np.sin(chi1 * DEG),
                       np.cos(twth1 / 2.*DEG) * np.cos(chi1 * DEG)])
        q2 = np.array([-np.sin(twth2 / 2.*DEG),
                       np.cos(twth2 / 2.*DEG) * np.sin(chi2 * DEG),
                       np.cos(twth2 / 2.*DEG) * np.cos(chi2 * DEG)])
...
#         nq2tilted_perp = np.sqrt(np.dot(q2tilted_perp, q2tilted_perp))
        ...
        if nq1tilted_perp <= 0.0001:
            ...
            angle = 1 / DEG * np.arcsin(np.dot(qaxis,
                        np.cross(q1tilted_perp / nq1tilted_perp,
...
#         print "now ready to rotate"
    ...
    def Annotate_exp(self, event):
        ...
        if self.datatype == '2thetachi':
            ...
            xtol = 0.05
            ytol = .05
            xdata, ydata, annotes = (self.data_gnomonx, self.data_gnomony,
                                     list(zip(self.Data_index_expspot, self.Data_I)))
        ...
        for x, y, a in self._dataANNOTE_exp:
            if (clickX - xtol < x < clickX + xtol) and (clickY - ytol < y < clickY + ytol) :
                annotes.append((GT.cartesiandistance(x, clickX, y, clickY),
                                x, y, a))
    ...
    def drawAnnote_theo(self, axis, x, y, annote):
        ...
        if (x, y) in self.drawnAnnotations_theo:
            ...
            # t = axis.text(x, y, "(%3.2f, %3.2f) - %s"%(x, y,annote), )  # par defaut
            if self.datatype == '2thetachi':
                t1 = axis.text(x + 1, y + 1, "#%d hkl=%s\nE=%.3f keV" % \
                               (annote[0], str(annote[1]), annote[2]), size=8)
            elif self.datatype == 'gnomon':
                t1 = axis.text(x + 0.02, y + 0.02, "#%d hkl=%s\nE=%.3f keV" % \
                               (annote[0], str(annote[1]), annote[2]), size=8)
            elif self.datatype == 'pixels':
                t1 = axis.text(x + 50, y + 50, "#%d hkl=%s\nE=%.3f keV" % \
                               (annote[0], str(annote[1]), annote[2]), size=8)
    ...
    def Annotate_theo(self, event):
        ...
        if self.datatype == '2thetachi':
            ...
            xdata, ydata, annotes = (self.data_theo[0], self.data_theo[1],
                        list(zip(np.arange(len(self.data_theo[0])), self.data_theo[2], self.data_theo[-1])))
...
if __name__ == '__main__':
    ...
    initialParameter['detectordiameter'] = 165.
